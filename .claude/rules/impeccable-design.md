...
> v1~v3 이 "어떻게 보이는가"라면 v4 는 **"한 화면에 얼마나 들어가는가"**다.
> 근거는 실측이다 — 공고 상세 '관리' 470→265px · 프로필 수정 706→325px ·
> '지금 할 일' 170→94px (커밋 `ba3d08a77`).

## 34. 세로 픽셀은 예산이다

iPhone 14 Pro 기준 852pt 에서 상단 세이프에어리어(59)와 하단 탭바(83)를 빼면
**710px** 가 첫 화면이다. 이 예산을 무엇이 쓰는지 모르면 밀도 논의는 취향 싸움이 된다.

**측정 먼저.** 화면을 고칠 때는 Before 픽셀을 재고, 고친 뒤 다시 잰다. "좀 짧아진 것
같다"는 결과가 아니다. `ba3d08a77` 이 470→265 처럼 숫자를 남긴 방식을 따른다.

### 34-1. 읽기 전용 라벨-값은 `InfoRow` 한 줄

라벨을 위에 두고 값을 박스에 담으면 한 항목이 **76px**(라벨 20 + 박스 44 + 여백 12)이다.
다섯 개면 380px — 화면 절반이다. 값을 오른쪽으로 보내 한 줄(**44px**)로 접는다.

수정할 수 없는 값에 입력창 모양을 주는 것부터가 거짓 어포던스다. `@/components/ui` 의
`InfoRow` 를 쓴다.

### 34-2. 진입점 3개 이상은 `ActionTileGrid`

"한 진입점 = 한 행"은 칸마다 세로 70px 을 먹는다(제목 + 설명 두 줄). 여섯 개면 420px.
2열 타일로 내면 절반이 된다 — `@/components/ui/ActionTileGrid`.

설명문이 "지원자 목록을 확인합니다." 처럼 제목의 되풀이라면 **화면에서 지우고 접근성
라벨에만 남긴다**. 시각 사용자에겐 제목만으로 충분하고, 스크린리더 사용자에겐 목적지를
가늠할 문장이 필요하다 — 지우는 게 아니라 옮기는 것이다.

### 34-3. 리스트 화면의 고정 영역은 뷰포트의 30% 이내

리스트가 본체인 화면에서 헤더·필터·CTA·배너가 쌓이면 **리스트가 시작되기도 전에**
예산이 소진된다. `내 스케줄` 이 그랬다 — 고정 헤더 스택이 리스트 뷰포트를 350px 로
좁히고 있었고, 압축의 핵심은 카드가 아니라 그 스택이었다.

710px 기준 **210px** 가 상한이다. 넘으면 다음 순서로 덜어낸다.

| 순서 | 무엇을 | 어디로 |
|---|---|---|
| 1 | 하루 한 번 누르는 풀폭 버튼 | 헤더 액션 아이콘 |
| 2 | 조건부로 나타나는 전용 행 | 인접 요소에 인라인 흡수 |
| 3 | 같은 성격의 별도 섹션 | 필터 탭의 한 갈래로 |
| 4 | 리스트 직전 여백 | 한 단계 축소(`mb-4`→`mb-2`) |

**주의**: 3번은 정보구조 변경이라 도메인 판단이 필요하다. 사장이 늘 보던 자리가
사라지면 "메뉴가 없어졌다"고 읽는다 — 이 앱에서 실제로 난 사고다
(`my-postings/[id]/index.tsx` 의 승격 카드 주석 참고). 진입점을 옮길 때는 **옮긴 자리에
같은 목적지를 남길지** 먼저 정한다.

### 34-4. 카드 푸터는 메타 행이 없을 때만

리스트 카드에 구분선 + 푸터를 달면 카드당 **27px** 가 붙는다. 네 장이면 108px.
푸터에 든 것이 지원자 수·상태 같은 메타라면 위 메타 행으로 올리고 구분선을 지운다.
푸터는 **탭 타깃이 따로 있을 때만** 자기 줄을 가질 자격이 있다.

### 34-5. 풀폭 바가 탭 타깃이 아니면 라벨이다

카드 전체가 이미 같은 곳으로 가는 `Pressable` 인데 그 안에 풀폭 CTA 바를 두면,
그 바는 두 번째 탭 타깃이 아니라 **라벨**이다. 라벨이라면 제목 옆자리로 충분하다
(170→94px). 자기 줄(44px)과 위 여백(10px)을 통째로 쓸 이유가 없다.

## v4 체크리스트 — 밀도(5항목)

- [ ] 이 화면의 고정 영역이 710px 예산의 30%(210px) 이내인가?
- [ ] 읽기 전용 라벨-값이 `InfoRow` 한 줄인가? (라벨 위 + 박스 아래 금지)
- [ ] 진입점 3개 이상이 `ActionTileGrid` 2열인가?
- [ ] 되풀이 설명문을 화면에서 지우고 접근성 라벨로 옮겼는가?
- [ ] Before/After 픽셀을 실제로 쟀는가? ("짧아진 것 같다" 금지)
