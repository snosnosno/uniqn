...
## RN 어댑터 (이 프로젝트 전용 — uniqn-mobile)

이 프로젝트는 React Native(Expo 55)다. 모션 스택은 **Reanimated 4 + react-native-gesture-handler + NativeWind 4**이며 CSS가 아니다. 리뷰 시 아래 번역표를 적용하라.

| 원문의 웹 개념 | 이 코드베이스의 등가물 |
| --- | --- |
| `transition` / `@keyframes` | `withTiming` / `withSpring` / `entering=`·`exiting=` 프리셋 |
| `cubic-bezier(...)` | `Easing.bezier(...)` — 곡선 값은 그대로 재사용 |
| spring `bounce`+`duration` | `withSpring({ damping, stiffness, mass, velocity })` |
| `:active` 스케일 피드백 | `Pressable` pressed 상태 + `scale(0.97)` |
| `transform-origin` | RN 미지원 — translate+scale 조합으로 시뮬레이션 필요 |
| `prefers-reduced-motion` | reanimated `useReducedMotion()` / `AccessibilityInfo.isReduceMotionEnabled` |
| GPU 속성 규칙 | UI-thread worklet에서 `transform`/`opacity`만. `width`/`height` 등 레이아웃 속성 애니메이션·JS-thread `Animated`(useNativeDriver:false)는 성능 결함 |
| `@media (hover)` 게이트 | 네이티브에 hover 없음 — RN-web 배포 표면에서만 검토 |
| keyframes 중단 불가 문제 | RN `LayoutAnimation`·`entering/exiting` 프리셋도 중단 불가 — 빠르게 재트리거되는 요소엔 `withSpring`/`withTiming` 재타게팅 사용 |

스캔 패턴: `withSpring|withTiming|entering=|exiting=|LayoutAnimation|Animated\.(timing|spring)|Easing\.`
CSS 전용 트리거(`transition: all`, `@starting-style`, WAAPI, Radix 변수, `@media` 쿼리)는 이 코드베이스에서 발화하지 않는다 — 부재를 결함으로 보고하지 말 것.
