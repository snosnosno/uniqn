...
- 전부 pass 확인 후 머지. **pending 상태에서 머지하지 말 것.** E2E 는 10분 넘게 걸린다 — `--watch` 는 셸 타임아웃에 걸리니 `sleep` + 재조회로 폴링한다.
- 실패가 이 PR과 무관한 기존 red 인지 반드시 구분한다(master 베이스라인부터 red 인 게이트가 있다 — knip 래칫 등).
- 이 레포는 **squash 저장소**다(rebase 금지).

🚨 **`gh pr merge` 가 `fatal: 'master' is already used by worktree at ...` 로 죽어도 머지는 성공했을 수 있다.**
그건 머지 후 **로컬 브랜치 전환** 단계에서 난 에러다. 실패로 단정하지 말고 상태를 실측한다:

```bash
gh pr view <번호> --json state,mergeCommit -q '.state + " " + .mergeCommit.oid'
```

- 머지 후 원격 브랜치 삭제.

### 1-5. 브랜치가 뒤처졌으면 — 파리티는 **머지 시점에 재산정**

병렬 세션이 그 사이 머지되면 `origin/master` 를 병합해야 한다. 이때 파리티 기대값이 거의 항상 충돌한다.

🔑 **브랜치를 딸 때 계산한 값은 이미 낡았다.** `기존값 ± 내 변화량` 이 아니라
**`새 master 값 ± 내 변화량`** 으로 다시 계산한다. (실측: 183−1=182 로 적었는데 #370 이
먼저 머지되며 master 가 185 가 되어 185−1=**184** 로 합류)

충돌 해결 시 **양쪽 이력 주석을 모두 보존**한다 — 나중에 숫자가 어디서 왔는지 추적할 유일한 단서다.
...
| push 타임아웃을 실패로 오판 | 같은 브랜치를 두 번 올리려다 꼬임 (pre-push 훅이 quality 를 돌려 5분 넘게 걸린다 — timeout 넉넉히) |
| `gh pr merge` 에러를 머지 실패로 오판 | 이미 머지된 PR 을 다시 머지하려 함 |
| 브랜치 시점 파리티 값을 그대로 사용 | 병렬 세션 머지 후 기대값이 틀려 CI red |
| 커밋 메시지 백틱 | 문장 소실 |
