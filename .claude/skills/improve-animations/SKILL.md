...
이 프로젝트는 React Native(Expo 55) + **Reanimated 4 + gesture-handler + NativeWind 4**다. CSS 기준 지침을 아래처럼 번역해 적용하라.

- **Phase 1 스윕 패턴 교체**: `transition`·`@keyframes` 대신 `withSpring|withTiming|withDelay|entering=|exiting=|LayoutAnimation|Animated\.(timing|spring)|Easing\.|useAnimatedStyle` 를 grep 하라. 대상 루트는 `uniqn-mobile/src/`.
- **값 번역**: cubic-bezier 곡선 → `Easing.bezier(...)` 그대로. spring bounce/duration → `withSpring({ damping, stiffness, velocity })`. `transform-origin`은 RN 미지원(translate+scale 시뮬레이션). reduced motion은 `useReducedMotion()`(reanimated).
- **성능 기준 번역**: "GPU 속성만" = UI-thread worklet에서 `transform`/`opacity`만 애니메이트. 레이아웃 속성(`width`/`height`/`padding`) 애니메이션과 JS-thread 구동(`useNativeDriver: false`)이 이 플랫폼의 성능 결함이다.
- **계획 산출 경로**: `plans/` 대신 이 레포 관례인 `docs/planning/animation-plans/`에 `NNN-slug.md`로 작성하라.
- CSS 전용 결함(`transition: all`, WAAPI 부재 등)은 발화 대상 아님. RN-web 배포 표면(CF Pages) 관련 발견은 별도 표기.
