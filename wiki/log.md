...
## [2026-07-31] note | 시트 드래그 닫기 부재·길찾기 오안내·연락처 국가코드 오표기 3종 수정 (PR#366)

실기기 스크린샷 제보 4건. DB 마이그레이션 없음.

- **하단 시트가 당겨도 안 닫혔다** — `Modal(position='bottom')` 에 손잡이 바만 그려 두고 팬 제스처가 **처음부터 없었다**. 당기면 안쪽 ScrollView 만 반응해 "내용만 내려가는" 증상이 iOS 버그로 읽혔다. 본문 스크롤이 맨 위일 때만 팬이 시트를 잡고(그 외엔 스크롤에 양보), 110px 또는 900px/s 초과 시 화면 밖으로 내보낸 뒤 닫는다. **RNModal 안에는 `GestureHandlerRootView` 를 한 번 더 깔아야 한다**(별도 윈도우라 앱 루트의 것이 닿지 않음, RNGH 제약). 적용 범위는 `Modal` 하단 시트뿐 — `SheetModal`(전체화면 시트)에는 여전히 드래그가 없다.
- **길찾기가 엉뚱한 곳으로 안내했다** — 주문서 PlaceSheet 의 "주소" 입력은 canonical 에서 `district` 로 흡수되는데(`draftAdapter`: `district = district ?? address`) 스케줄 상세는 `detailedAddress` 만 읽었고, **`detailedAddress` 를 채우는 UI 는 주문서에 없다**. 주소를 적어 둔 공고도 화면엔 장소명만 남아 지도가 그 이름('홈')으로 검색했다. `ScheduleEvent.locationAddress` 로 배선하고, 안내 근거가 없으면 버튼 대신 사유를 밝힌다(장소명 검색은 없는 것보다 나쁘다).
- **구인자 연락처가 `821-0980-0903`** — 앱 전체가 `formatE164ToDisplay` 를 쓰는데 스케줄 탭 3곳만 `formatPhoneNumber`(국가코드 미인지)라 E.164 가 3-4-4 로 잘리고 마지막 자리가 유실됐다. 표시 전용 `formatPhoneForDisplay` 로 단일화.
- 스태프 관리 상단 버튼 2행 → 1행(추가 2 : 새로고침 1).

재사용 교훈 3:

- **어포던스만 그려두고 동작이 없으면 사용자는 "고장"으로 읽는다.** "아이폰에서 안 닫힌다"는 제보를 플랫폼 이슈로 넘겨짚을 뻔했다 — 코드엔 제스처가 **아예 없었다**.
- **입력 필드명과 소비 필드명이 다르면 데이터는 조용히 사라진다.** 저장은 되는데 읽는 쪽이 다른 키를 본다 — 에러가 없어 발견이 늦다.
