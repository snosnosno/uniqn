...
export const resetFirebaseConnection = async (): Promise<void> => {
  ...
  try {
    ...
    logger.error(
      '❌ Error resetting Firebase connection',
      error instanceof Error ? error : new Error(String(error)),
      { operation: 'resetFirebaseConnection' }
    );
  }
};
...
export const forceFirebaseReconnection = async (): Promise<void> => {
  ...
  try {
    ...
    logger.error(
      '❌ Error forcing Firebase reconnection',
      error instanceof Error ? error : new Error(String(error)),
      { operation: 'forceFirebaseReconnection' }
    );
...
export const checkFirebaseConnection = async (): Promise<boolean> => {
  try {
    ...
    logger.error(
      ...
      error instanceof Error ? error : new Error(String(error)),
      { operation: 'checkFirebaseConnection' }
    );
    return false;
  }
};
