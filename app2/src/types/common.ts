 ...
 * WorkLog 타입 (공통 버전)
 *
 * @deprecated 이 타입은 하위 호환성을 위해 유지됩니다.
 * 새로운 코드에서는 `types/unified/workLog.ts`의 `UnifiedWorkLog`를 사용하세요.
 *
 * @description 기본 WorkLog 타입입니다. 시간 필드가 string으로 정의되어 있어
 * Timestamp 기반의 UnifiedWorkLog보다 제한적입니다.
 *
 * 마이그레이션 가이드:
 * ```typescript
 * // ❌ 기존 방식 (deprecated)
 ...
 * const log: UnifiedWorkLog = { ... };
 * ```
 *
 * @see types/unified/workLog.ts - UnifiedWorkLog (SSOT, 권장)
 ...
 *
 * @deprecated 이 타입은 하위 호환성을 위해 유지됩니다.
 * 새로운 코드에서는 `types/attendance.ts`의 `AttendanceRecord`를 사용하세요.
 *
 * @description 기본 AttendanceRecord 타입입니다. 완전한 기능 세트는
 * types/attendance.ts에 정의되어 있습니다.
 *
 * 마이그레이션 가이드:
 * ```typescript
 * // ❌ 기존 방식 (deprecated)
 ...
 * ```
 *
 * @see types/attendance.ts - AttendanceRecord (SSOT, 권장)
 */
