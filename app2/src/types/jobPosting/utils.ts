...
export class JobPostingUtils {
   ...
   * @returns TimeSlot 배열
   */
  static getTimeSlotsForDate(jobPosting: JobPosting, date: string): TimeSlot[] {
  ...
  static getConfirmedStaffCount(
    ...
    if (!matchingRequirement) {
      matchingRequirement = jobPosting.dateSpecificRequirements?.find((req) => {
        ...
        const endDate = firstTimeSlot?.duration?.endDate
          ? timestampToLocalDateString(firstTimeSlot.duration.endDate)
          : null;
    ...
    const isMultiDayRange = matchingRequirement
      ? matchingRequirement.timeSlots?.[0]?.duration?.type === 'multi'
      : false;
    ...
    const matchingStaff = jobPosting.confirmedStaff.filter((staff) => {
      ...
      // 기본 조건: 역할, 시간대, 날짜 매칭
      const basicMatch =
        staff.timeSlot === timeSlot && staff.role === role && staffDate === targetDate;
   ...
   */
  static isRoleFull(jobPosting: JobPosting, timeSlot: string, role: string, date: string): boolean {
    let requiredCount = 0;
  ...
  static getApplicantsByDate(applicants: Applicant[], date: string): Applicant[] {
    return applicants.filter((applicant) => applicant.assignedDate === date);
  }
    ...
    jobPosting.dateSpecificRequirements?.forEach((dateReq) => {
      ...
      dateReq.timeSlots.forEach((ts) => {
        ts.roles.forEach((role) => {
          ...
          totalConfirmed += this.getConfirmedStaffCount(jobPosting, dateStr, ts.time, role.name);
        });
      });
    });
  ...
  static getRequirementProgress(
    jobPosting: JobPosting
  ): Map<string, { required: number; confirmed: number }> {
