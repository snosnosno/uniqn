...
const StaffCard: React.FC<StaffCardProps> = React.memo(
  ({
    ...
    const memoizedAttendanceData = useMemo(() => {
      ...
      if (!/^\d{4}-\d{2}-\d{2}$/.test(dateString)) {
        logger.warn('⚠️ StaffCard - assignedDate 파싱 실패:', {
          component: 'StaffCard',
          data: {
            staffId: staff.id,
      ...
      if (workLog?.scheduledStartTime) {
        try {
          if (typeof workLog.scheduledStartTime === 'string') {
            ...
            typeof workLog.scheduledStartTime === 'object' &&
            'toDate' in workLog.scheduledStartTime
          ) {
            // Timestamp인 경우
      ...
      if (workLog?.scheduledEndTime) {
        try {
          if (typeof workLog.scheduledEndTime === 'string') {
            ...
            typeof workLog.scheduledEndTime === 'object' &&
            'toDate' in workLog.scheduledEndTime
          ) {
            // Timestamp인 경우
                  ...
                  <div className="flex-1 min-w-0">
                    <AttendanceStatusPopover
                      ...
                      currentStatus={
                        ...
                        memoizedAttendanceData.attendanceRecord?.status ||
                        'not_started'
                      }
                      staffId={staff.id}
                      staffName={staff.name || ''}
  ...
  (prevProps, nextProps) => {
    ...
    const prevWorkLog = prevProps.getStaffWorkLog
      ? prevProps.getStaffWorkLog(prevProps.staff.id, dateString)
      : null;
    const nextWorkLog = nextProps.getStaffWorkLog
      ? nextProps.getStaffWorkLog(nextProps.staff.id, dateString)
      : null;

    // workLog의 주요 시간 필드 변경 체크
    if (
      prevWorkLog?.scheduledStartTime !== nextWorkLog?.scheduledStartTime ||
    ...
    const prevAttendanceRecords = prevProps.attendanceRecords.filter(
      (r) => r.staffId === prevProps.staff.id || r.workLog?.staffId === prevProps.staff.id
    );
    const nextAttendanceRecords = nextProps.attendanceRecords.filter(
      (r) => r.staffId === nextProps.staff.id || r.workLog?.staffId === nextProps.staff.id
    );
