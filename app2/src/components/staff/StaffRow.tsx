...
const StaffRow: React.FC<StaffRowProps> = React.memo(
      ...
      if (currentWorkLog?.scheduledStartTime) {
        try {
          ...
          if (typeof currentWorkLog.scheduledStartTime === 'string') {
            ...
            currentWorkLog.scheduledStartTime &&
            typeof currentWorkLog.scheduledStartTime === 'object' &&
            'toDate' in currentWorkLog.scheduledStartTime
          ) {
      ...
      if (currentWorkLog?.scheduledEndTime) {
        try {
          if (typeof currentWorkLog.scheduledEndTime === 'string') {
            ...
            currentWorkLog.scheduledEndTime &&
            typeof currentWorkLog.scheduledEndTime === 'object' &&
            'toDate' in currentWorkLog.scheduledEndTime
          ) {
            // Timestamp인 경우 - 더 정확한 시간 포맷 사용
      ...
      >
          ...
          <div className="text-sm text-gray-900 dark:text-gray-100 space-y-1">
            {staff.phone && (
              <div className="flex items-center">
                <svg
                  className="w-3 h-3 mr-1 text-gray-400 dark:text-gray-500 dark:text-gray-400 dark:text-gray-500"
                  fill="currentColor"
                  viewBox="0 0 20 20"
                >
        ...
        >
          <AttendanceStatusPopover
            workLogId={
              memoizedAttendanceData.realWorkLogId ||
              memoizedAttendanceData.attendanceRecord?.workLogId ||
              memoizedAttendanceData.workLogId
            }
            ...
            size="sm"
            actualStartTime={
              currentWorkLog?.actualStartTime ||
              memoizedAttendanceData.attendanceRecord?.workLog?.actualStartTime
            }
            actualEndTime={
              currentWorkLog?.actualEndTime ||
              memoizedAttendanceData.attendanceRecord?.workLog?.actualEndTime
            }
  ...
  (prevProps, nextProps) => {
    ...
    const prevRecord = findAttendanceRecord(
      prevProps.attendanceRecords,
      prevProps.staff.id,
      dateString
    );
    const nextRecord = findAttendanceRecord(
      nextProps.attendanceRecords,
      nextProps.staff.id,
      dateString
    );
    ...
    if (
      JSON.stringify(prevRecord?.workLog?.actualStartTime) !==
        JSON.stringify(nextRecord?.workLog?.actualStartTime) ||
      JSON.stringify(prevRecord?.workLog?.actualEndTime) !==
        JSON.stringify(nextRecord?.workLog?.actualEndTime)
    ) {
