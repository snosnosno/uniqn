  ...
  const processAssignments = () => {
    ...
    assignments.forEach((assignment) => {
      ...
      const checkMethod =
        ...
        (assignment.dates && assignment.dates.length > 1 && assignment.isGrouped
          ? 'group'
          : 'individual');

      // 그룹 선택과 개별 선택 구분 표시
      const isGroupSelection: boolean =
        checkMethod === 'group' || Boolean(assignment.roles && assignment.roles.length > 0);
      ...
      if (checkMethod === 'group') {
        ...
        let existingGroup = dateGroups.find(
          (group) => group.dateKey === dateKey && group.checkMethod === 'group'
        );
        ...
        assignment.dates.forEach((date: DateValue) => {
          ...
          // 같은 날짜의 기존 그룹 찾기
          let existingGroup = dateGroups.find(
            (group) => group.dateKey === dateKey && group.checkMethod === 'individual'
          );
          ...
          // 같은 시간대의 기존 슬롯 찾기
          let existingTimeSlot = existingGroup.timeSlots.find(
            (slot) => slot.timeSlot === assignment.timeSlot
          );
              ...
              <div key={slotIndex}>
                {status === 'confirmed' ? (
                  ...
                  <div className="text-sm text-gray-700 dark:text-gray-300 font-medium">
                    📅 {group.dateDisplay} ⏰ {timeSlot.timeSlot} 👤{' '}
                    {timeSlot.roles
                      .filter((role) => role)
                      .map((role) => t(`roles.${role}`) || role)
                      .join(', ')}
                  </div>
                ) : (
                          ...
                          <div className="flex flex-wrap gap-1">
                            {timeSlot.roles
                              ...
                              .map((role, roleIndex) => (
                                <span
                                  key={roleIndex}
                                  className="bg-purple-50 dark:bg-purple-900/30 text-purple-700 dark:text-purple-300 px-2 py-0.5 rounded text-sm"
                                >
                                  {t(`roles.${role}`) || role}
                                </span>
                              ))}
