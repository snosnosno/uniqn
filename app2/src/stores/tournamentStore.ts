      ...
      immer((set, get) => ({
        ...
        addParticipant: (name) =>
          set((state) => {
            ...
            let targetTable = state.tables.find(
              (t) => t.players.length < state.settings.seatsPerTable
            );
        ...
        updateChipCount: (participantId, chipCount) =>
          set((state) => {
            const participant = state.participants.find((p) => p.id === participantId);
            if (participant) {
        ...
        rebuyParticipant: (participantId) =>
          set((state) => {
            const participant = state.participants.find((p) => p.id === participantId);
        ...
        moveParticipant: (participantId, toTableNumber, toSeatNumber) =>
          set((state) => {
            const participant = state.participants.find((p) => p.id === participantId);
