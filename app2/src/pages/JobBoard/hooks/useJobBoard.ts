...
export const useJobBoard = () => {
  ...
  const [preQuestionAnswers, setPreQuestionAnswers] = useState<Map<string, PreQuestionAnswer[]>>(
    new Map()
  );
  ...
  const {
    applications,
    jobPostings: jobPostingsFromStore,
    loading: unifiedDataLoading,
  } = useUnifiedData();
  ...
  const handleApply = async () => {
    ...
    try {
      ...
      const assignments = Array.from(groupedAssignments.entries())
        .map(([_groupKey, items]) => {
  ...
  const handleCancelApplicationConfirm = async () => {
    ...
    try {
      ...
      const qEventId = query(
        ...
        where('eventId', '==', cancelConfirmPostId)
      );
      const qPostId = query(
        collection(db, 'applications'),
        where('applicantId', '==', currentUser.uid),
        where('postId', '==', cancelConfirmPostId)
      );
  ...
  const handleOpenDetailModal = (postOrId: JobPosting | string) => {
    ...
    if (typeof postOrId === 'string') {
      ...
      if (!fullJobPosting) {
        logger.warn('JobPosting not found for postId', {
          component: 'useJobBoard',
          value: postOrId,
        });
