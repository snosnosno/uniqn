...
const ShiftSchedulePage: React.FC = () => {
  ...
  const availableDealers = useMemo(
    () =>
      (allStaff?.filter(
        (s) => Array.isArray(s.jobRole) && s.jobRole.includes('Dealer')
      ) as ShiftDealer[]) || [],
    [allStaff]
  );
  ...
  useEffect(() => {
    const loadSettings = () => {
      try {
        ...
        logger.error('설정 로드 실패:', error instanceof Error ? error : new Error(String(error)), {
  ...
  const formatDate = (dateInput: string | Date | null | undefined) => {
    ...
    try {
      ...
      logger.error(
        ...
        { component: 'ShiftSchedulePage', data: { dateInput } }
      );
      return String(dateInput); // Convert to string as fallback
    }
  };
  ...
  return (
    <>
              ...
              {workLogsGenerated ? (
                <div className="flex items-center gap-2 text-green-600">
                  ...
                  <span className="text-sm font-medium">
                    {t('shiftSchedule.workLogsGenerated')}
                  </span>
                </div>
              ) : (
                <div className="flex items-center gap-2 text-orange-600">
                  ...
                  <span className="text-sm font-medium">
                    {t('shiftSchedule.workLogsNotGenerated')}
                  </span>
                </div>
              )}
            </div>
          ...
          <div className="xl:col-span-3">
            <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-md">
              <h2 className="text-xl font-semibold mb-4 text-blue-600 dark:text-blue-400 flex items-center">
                ...
                {schedule ? (
                  <span className="ml-2 text-sm font-normal text-gray-500 dark:text-gray-400 dark:text-gray-500">
                    ({schedule.timeInterval}
                    {t('shiftSchedule.minuteInterval')})
                  </span>
                ) : null}
              </h2>
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex items-center gap-4 text-sm text-gray-600 dark:text-gray-300">
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      <span>
                        {dealers.length}
                        {t('shiftSchedule.assignedDealer')}
                      </span>
                    </div>
                  </div>
              ...
              <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-md">
                ...
                <div className="space-y-3 max-h-64 overflow-y-auto">
                  {dealers.map((dealer) => (
                    <div
                      key={dealer.id}
                      className="flex items-center bg-blue-50 dark:bg-blue-900/20 p-3 rounded-lg shadow-sm"
                    >
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-sm text-gray-500 dark:text-gray-400 dark:text-gray-500">
                          {t('shiftSchedule.startTime')}: {dealer.startTime}
                        </p>
                      </div>
                      <button
                        disabled={true}
            ...
            <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-md dark:shadow-gray-900/50">
              ...
              <div className="space-y-3 max-h-64 overflow-y-auto">
                {dealersNotInSchedule.map((dealer) => (
                  <div
                    key={dealer.id}
                    className="flex items-center bg-gray-50 dark:bg-gray-700 p-3 rounded-lg shadow-sm"
                  >
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-sm text-gray-500 dark:text-gray-400 dark:text-gray-500">
                        {Array.isArray(dealer.jobRole) ? dealer.jobRole.join(', ') : ''}
                      </p>
                    </div>
                    {schedule ? (
                      <button
                ...
                {dealersNotInSchedule.length === 0 && (
                  <p className="text-sm text-gray-500 dark:text-gray-400 text-center py-4">
                    {schedule
                      ? t('shiftSchedule.allDealersAssigned')
                      : t('shiftSchedule.noDealersAvailable')}
                  </p>
                )}
              </div>
            </div>
      ...
      {/* 설정 모달 */}
      {isSettingsModalOpen ? (
        <div className="fixed inset-0 bg-black dark:bg-black bg-opacity-50 dark:bg-opacity-50 flex items-center justify-center z-50">
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <label className="cursor-pointer label">
                  <span className="label-text text-sm">
                    {t('shiftSchedule.conflictNotifications')}
                  </span>
                  <input
                    type="checkbox"
                    className="checkbox checkbox-sm"
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    handleSettingChange('defaultWorkTime', parseInt(e.target.value) || 4)
                  }
                />
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    handleSettingChange('defaultBreakTime', parseInt(e.target.value) || 30)
                  }
                />
              </div>
            </div>
