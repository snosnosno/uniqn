...
const TablesPage: React.FC = () => {
  ...
  } = useSettings(state.userId, state.tournamentId);
  ...
  const [waitingParticipantForAssignment, setWaitingParticipantForAssignment] =
  ...
  const waitingParticipants = useMemo(() => {
    ...
    return participants.filter(
      (p) => p.status === 'active' && (p.tableNumber === undefined || p.tableNumber === null)
    );
  ...
  const handleManualAssignConfirm = async (tableId: string, seatIndex: number) => {
    ...
    try {
      ...
      await runTransaction(db, async (transaction) => {
        const tableRef = doc(
          db,
          `users/${state.userId}/tournaments/${actualTournamentId}/tables`,
          tableId
        );
        ...
        const participantRef = doc(
          db,
          `users/${state.userId}/tournaments/${actualTournamentId}/participants`,
          waitingParticipantForAssignment.id
        );
  ...
  if (tablesLoading || participantsLoading || settingsLoading)
    return (
      <div className="p-4 text-gray-900 dark:text-gray-100">{t('common.messages.loading')}</div>
    );
  if (tablesError || participantsError)
    return (
      <div className="p-4 text-red-500 dark:text-red-400">
        {t('tables.error')} {tablesError?.message || participantsError?.message}
      </div>
    );
            ...
            <div className="flex justify-between items-center text-gray-600 dark:text-gray-300">
              <div className="flex items-center space-x-6">
                <div className="flex items-center">
                  <FaThList className="w-5 h-5 mr-2 text-blue-500 dark:text-blue-400" />{' '}
                  {t('tables.infoTables')} <span className="font-bold ml-1">{tables.length}</span>
                </div>
                <div className="flex items-center">
                  <FaUserPlus className="w-5 h-5 mr-2 text-green-500 dark:text-green-400" />{' '}
                  {t('tables.infoEmptySeats')}{' '}
                  <span className="font-bold ml-1">{totalEmptySeats}</span>
                </div>
              </div>
              <div className="flex items-center space-x-2">
                ...
                >
                  {t('tables.labelMaxSeats')}
                </label>
                <select
                  id="max-seats"
                  value={settings.maxSeatsPerTable || 9}
                  onChange={(e) => handleMaxSeatsChange(parseInt(e.target.value, 10))}
                  className="select select-bordered bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 border-gray-300 dark:border-gray-600"
                  disabled={state.tournamentId === 'ALL'}
                >
                  {Array.from({ length: 8 }, (_, i) => i + 4).map((num) => (
                    <option key={num} value={num} className="bg-white dark:bg-gray-700">
                      {num}
                    </option>
                  ))}
                </select>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                {tables.map((table) => {
                  ...
                  const tournamentColor =
                    table.tournamentColor ||
                    getTournamentColorById(table.tournamentId, tournaments) ||
                    UNASSIGNED_COLOR;
          ...
          {isMoveSeatModalOpen && selectedPlayer?.participant ? (
            <MoveSeatModal
              ...
              currentTournamentName={
                tournaments.find((t) => t.id === selectedPlayer.table.tournamentId)?.name
              }
            />
          ) : null}

          {/* 대기 참가자 수동 배정 모달 */}
          {isWaitingAssignmentModalOpen && waitingParticipantForAssignment ? (
            <MoveSeatModal
              ...
              >
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setEditingParticipant((p) => (p ? { ...p, name: e.target.value } : null))
                    }
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setEditingParticipant((p) => (p ? { ...p, phone: e.target.value } : null))
                    }
                    className="input-field w-full"
                  />
                </div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setEditingParticipant((p) =>
                        p ? { ...p, chips: Number(e.target.value) } : null
                      )
                    }
                    className="input-field w-full"
                  />
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setEditingParticipant((p) =>
                        p
                          ? { ...p, status: e.target.value as 'active' | 'busted' | 'no-show' }
                          : null
                      )
                    }
                    className="input-field w-full"
