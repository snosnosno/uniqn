...
export const useStaffWorkData = ({
  ...
  const staffWorkData = useMemo((): StaffWorkDataItem[] => {
    ...
    uniqueStaffMap.forEach((staff) => {
      ...
      allRoles.forEach((role) => {
        ...
        const validLogs = roleWorkLogs.filter((log) => {
          const hasValidStartTime =
            (log.scheduledStartTime &&
              log.scheduledStartTime !== '미정' &&
              log.scheduledStartTime !== '') ||
            (log.assignedTime && log.assignedTime !== '미정' && log.assignedTime !== '');
          const hasValidEndTime =
            log.scheduledEndTime && log.scheduledEndTime !== '미정' && log.scheduledEndTime !== '';
        ...
        // 역할별 기본급 계산
        const roleBasePay = calculateBasePay(
          roleWorkLogs,
          salaryType,
          salaryAmount,
          roleHours,
  ...
  const applyBulkAllowances = useCallback(
    (settings: BulkAllowanceSettings) => {
      ...
      staffWorkData.forEach((item) => {
        ...
        if (
          ...
          !settings.targetRoles.includes(item.role)
        ) {
          return;
        }
  ...
  const updateStaffAllowances = useCallback(
    (uniqueKey: string, allowances: EnhancedPayrollCalculation['allowances']) => {
