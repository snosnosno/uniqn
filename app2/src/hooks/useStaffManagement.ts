...
export const useStaffManagement = (
  ...
  // 메모이제이션된 날짜별 그룹화 토글
  const toggleDateExpansion = useCallback(
    (date: string) => {
      ...
      if (eventId) {
        localStorage.setItem(
          `staffManagement-${eventId}-expandedDates`,
          JSON.stringify(Array.from(newExpandedDates))
        );
      }
    },
  ...
  const getTimeSlotColor = useCallback((time: string | undefined): string => {
    ...
    if (hour >= 6 && hour < 12)
      return 'bg-yellow-100 dark:bg-yellow-900/30 text-yellow-800 dark:text-yellow-200'; // 오전
    if (hour >= 12 && hour < 18)
      return 'bg-blue-100 dark:bg-blue-900/30 text-blue-800 dark:text-blue-200'; // 오후
    if (hour >= 18 && hour < 24)
      return 'bg-purple-100 dark:bg-purple-900/30 text-purple-800 dark:text-purple-200'; // 저녁
  ...
  const filteredStaffData = useMemo(() => {
    ...
    if (filters.selectedRole !== 'all') {
      filtered = filtered.filter(
        (staff) => (staff.assignedRole || staff.role) === filters.selectedRole
      );
    }
  ...
  const groupedStaffData = useMemo((): GroupedStaffData => {
    ...
    // 날짜별 그룹화
    const grouped = filteredStaffData.reduce(
      (acc, staff) => {
        let date: string;
        ...
        if (!staff.assignedDate) {
          ...
          try {
            ...
            logger.error(
              'useStaffManagement 날짜 포맷 오류',
              error instanceof Error ? error : new Error(String(error)),
              { component: 'useStaffManagement', data: { assignedDate: staff.assignedDate } }
            );
            date = '날짜 오류';
          }
        }
  ...
  const getStaffCountByDate = useCallback(
    (date: string): number => {
