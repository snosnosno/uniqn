...
export const validateConfig = (): boolean => {
  const requiredVars = ['REACT_APP_FIREBASE_PROJECT_ID', 'REACT_APP_FIREBASE_AUTH_DOMAIN'];
  ...
  if (useProxy && !proxyUrl) {
    logger.error(
      '프록시 사용이 설정되었으나 REACT_APP_FIREBASE_PROXY_URL이 없습니다.',
      new Error('Missing proxy URL'),
      {
        component: 'firebase.config',
      }
    );
