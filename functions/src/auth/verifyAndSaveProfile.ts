...
import { verifyOTPServerSide } from "../utils/phoneVerification";
    ...
    withCallableGuard(
      ...
      async (request) => {
        try {
          ...
          // 선택 필드 검증
          const region = validateOptionalString(data.region, "지역", {
            max: 50,
          });
          const career = validateOptionalString(data.career, "이력", {
            max: 500,
          });
          const note = validateOptionalString(data.note, "기타사항", {
            max: 300,
          });
          const email = validateOptionalString(data.email, "이메일", {
            max: 100,
          });
          ...
          // ── 2. 전화번호 검증 (서버사이드 OTP / 기존 클라이언트 link 분기) ──
          ...
          if (data.verificationId && data.otpCode) {
            ...
            if (!apiKey) {
              logger.error("verifyAndSaveProfile: WEB_API_KEY 미설정", {
                uid,
              });
              throw new ValidationError(ERROR_CODES.VALIDATION_FORMAT, {
                userMessage: "서버 설정 오류입니다. 관리자에게 문의해주세요.",
              });
            }
            ...
            // 검증된 전화번호와 클라이언트 전달 전화번호 대조
            if (otpResult.phoneNumber !== clientPhoneE164) {
              logger.warn("verifyAndSaveProfile: OTP 검증 전화번호 불일치", {
                uid,
                otpPhone: maskPhone(otpResult.phoneNumber),
            ...
            try {
              ...
              if (errorCode === "auth/phone-number-already-exists") {
                throw new ValidationError(ERROR_CODES.VALIDATION_FORMAT, {
                  userMessage: "이미 다른 계정에 등록된 전화번호입니다.",
                });
              }
              throw updateError;
            }

            logger.info("verifyAndSaveProfile: 서버사이드 전화번호 설정 완료", {
              uid,
              phone: maskPhone(clientPhoneE164),
            });
          } else {
            // ── 2b. 기존 클라이언트 link 검증 모드 (하위 호환) ─────────────
            //
            // 클라이언트에서 이미 linkWithCredential로 전화번호를 link한 경우,
            // Firebase Auth의 phoneNumber를 확인합니다.
