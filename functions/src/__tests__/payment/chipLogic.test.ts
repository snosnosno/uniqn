...
describe('칩 로직 테스트', () => {
  ...
  describe('칩 지급 (grantChips)', () => {
    it('빨간칩 30개 지급 성공', async () => {
      ...
      expect(chipBalance.redChips).toBe(30);
      expect(chipBalance.blueChips).toBe(0);
      expect(chipBalance.redChipExpiry).toBeTruthy();
    });
    ...
    it('파란칩 30개 지급 성공', async () => {
      ...
      expect(chipBalance.redChips).toBe(0);
      expect(chipBalance.blueChips).toBe(30);
      expect(chipBalance.blueChipExpiry).toBeTruthy();
    });
    ...
    it('빨간칩 + 파란칩 순차 지급', async () => {
      ...
      expect(chipBalance.redChips).toBe(30);
      expect(chipBalance.blueChips).toBe(50);
    });
    ...
    it('동일 타입 칩 여러 번 지급 (누적)', async () => {
      ...
      expect(chipBalance.redChips).toBe(60);
    });
    ...
    it('칩 거래 내역이 기록됨', async () => {
      ...
      expect(transaction.chipType).toBe('red');
      expect(transaction.reason).toBe('테스트 결제');
    });
  });
  ...
  describe('칩 차감 (deductChips) - 우선순위', () => {
    it('파란칩만 있을 때 차감', async () => {
      ...
      expect(chipBalance.blueChips).toBe(40); // 50 - 10
      expect(chipBalance.redChips).toBe(0);
    });
    ...
    it('빨간칩만 있을 때 차감', async () => {
      ...
      expect(chipBalance.redChips).toBe(20); // 30 - 10
      expect(chipBalance.blueChips).toBe(0);
    });
    ...
    it('파란칩 + 빨간칩 모두 있을 때 파란칩 우선 차감', async () => {
      ...
      expect(chipBalance.blueChips).toBe(30); // 50 - 20 (파란칩 우선 차감)
      expect(chipBalance.redChips).toBe(30); // 변동 없음
    });
    ...
    it('파란칩 부족 시 빨간칩 차감', async () => {
      ...
      expect(chipBalance.blueChips).toBe(0); // 10 전부 차감
      expect(chipBalance.redChips).toBe(15); // 30 - 15 (나머지 차감)
    });
    ...
    it('칩 부족 시 실패', async () => {
      ...
      expect(result.message).toContain('칩이 부족합니다');

      // 칩 잔액이 변경되지 않았는지 확인
      const chipBalanceDoc = await db.collection('chipBalance').doc(testUserId).get();
      const chipBalance = chipBalanceDoc.data()!;

      expect(chipBalance.redChips).toBe(10); // 변동 없음
    });
    ...
    it('칩 잔액 문서가 없을 때 실패', async () => {
      ...
      expect(result.success).toBe(false);
      expect(result.message).toContain('칩 잔액 문서가 존재하지 않습니다');
    });
    ...
    it('칩 차감 거래 내역이 기록됨', async () => {
      ...
      expect(transaction.reason).toBe('칩 사용');
      expect(transaction.balance.redChips).toBe(20);
    });
  });
  ...
  describe('칩 만료 (expireChips)', () => {
    it('만료되지 않은 칩은 그대로 유지', async () => {
      ...
      expect(result.redExpired).toBe(0);
      expect(result.blueExpired).toBe(0);
    ...
    it('빨간칩 만료 시 0으로 설정', async () => {
      ...
      expect(result.redExpired).toBe(30);

      const chipBalanceDoc = await db.collection('chipBalance').doc(testUserId).get();
      const chipBalance = chipBalanceDoc.data()!;

      expect(chipBalance.redChips).toBe(0);
      expect(chipBalance.redChipExpiry).toBeNull();
    });
    ...
    it('파란칩 만료 시 0으로 설정', async () => {
      ...
      expect(result.blueExpired).toBe(50);

      const chipBalanceDoc = await db.collection('chipBalance').doc(testUserId).get();
      const chipBalance = chipBalanceDoc.data()!;

      expect(chipBalance.blueChips).toBe(0);
      expect(chipBalance.blueChipExpiry).toBeNull();
    });
    ...
    it('칩 만료 거래 내역이 기록됨', async () => {
      ...
      expect(transaction.chipType).toBe('red');
      expect(transaction.amount).toBe(30);
      expect(transaction.reason).toContain('빨간칩 만료');
    });
  });
  ...
  describe('통합 시나리오', () => {
    it('결제 → 구독 → 사용 → 만료 전체 플로우', async () => {
      ...
      expect(chipBalance.redChips).toBe(30); // 변동 없음
      expect(chipBalance.blueChips).toBe(30); // 50 - 20
      ...
      expect(expired.blueExpired).toBe(30);

      // 6. 최종 칩 잔액 확인
      chipBalanceDoc = await db.collection('chipBalance').doc(testUserId).get();
      chipBalance = chipBalanceDoc.data()!;

      expect(chipBalance.redChips).toBe(30); // 빨간칩만 남음
      expect(chipBalance.blueChips).toBe(0); // 파란칩 만료
    ...
    it('파란칩 소진 후 빨간칩 사용', async () => {
      ...
      expect(chipBalance.blueChips).toBe(0);
      expect(chipBalance.redChips).toBe(10); // 30 - 20
    });
  });
  ...
  describe('엣지 케이스', () => {
    ...
    it('매우 큰 칩 지급 (100,000개)', async () => {
      ...
      expect(chipBalance.redChips).toBe(100000);
    });
    ...
    it('동시에 여러 칩 차감 시도 (트랜잭션 안전성)', async () => {
      await grantChips(testUserId, 100, 'red', '결제');

      // 동시에 10개씩 10번 차감 시도
      const promises = Array.from({ length: 10 }, () =>
        deductChips(testUserId, 10, '동시 사용')
      );
      ...
      expect(chipBalance.redChips).toBe(0); // 100 - (10 x 10)
    });
  });
});
