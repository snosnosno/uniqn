 ...
 */
export const rejectJobPosting = onCall(async (request) => {
  // 1. 인증 체크
  if (!request.auth) {
  ...
  // 3. 파라미터 검증
  const { postingId, reason } = request.data;
  ...
  if (!reason || typeof reason !== 'string' || reason.trim().length < 10) {
    ...
    throw new HttpsError('invalid-argument', '거부 사유는 최소 10자 이상이어야 합니다');
  }
  ...
  try {
    ...
    if (posting?.postingType !== 'tournament') {
      ...
      throw new HttpsError('invalid-argument', '대회 공고만 거부 가능합니다');
    }
    ...
    throw new HttpsError('internal', '공고 거부 중 오류가 발생했습니다');
  }
});
