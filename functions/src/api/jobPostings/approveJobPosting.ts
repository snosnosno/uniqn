 ...
 */
export const approveJobPosting = functions
  ...
  .https.onCall(async (data: { postingId: string }, context) => {
    ...
    try {
      ...
      if (posting?.postingType !== 'tournament') {
        ...
        throw new functions.https.HttpsError('invalid-argument', '대회 공고만 승인 가능합니다');
      }
      ...
      await postingRef.update({
        ...
        'tournamentConfig.approvedBy': context.auth.uid,
      ...
      throw new functions.https.HttpsError('internal', '공고 승인 중 오류가 발생했습니다');
    }
  });
