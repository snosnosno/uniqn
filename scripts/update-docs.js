...
function updateDocumentFile(filePath, packageVersion) {
  ...
  if (changeCount > 0) {
    ...
    if (success) {
      console.log(
        `✅ ${path.basename(filePath)} 업데이트 완료 (${changeCount}개 변경)`,
      );
      return true;
    }
  } else {
...
function checkDocumentConsistency() {
  ...
  if (readmeContent && packageVersion) {
    const versionMatch = readmeContent.match(
      /\*\*버전\*\*:\s*(v?\d+\.\d+\.\d+)/,
    );
    const readmeVersion = versionMatch
  ...
  if (todoContent) {
    ...
    const sprintMatch = todoContent.match(
      /(\d{4})년\s*(\d{1,2})월\s*(\d{1,2})일\s*-\s*(\d{1,2})일/,
    );
