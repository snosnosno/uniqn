-- ============================================================
-- status 패치 축 신설 — 감사 data-01 (4번째 이력 jsonb Lost Update) 의 **서버 절반**
--
-- 클라 `ConfirmedStaffRepository.updateStatus` 가 지금 클라에서 하는 일 —
-- 타임스탬프 파생(`buildStatusTimestampPatch`) · 감사 이력 조립(`buildManualStatusAudit`) ·
-- `end_time_source`/`edited_by` 보정 — 을 이 RPC 가 `status` 패치 키 하나로 흡수한다.
-- 클라는 `modification_history` 를 **읽어서 spread 로 덧붙인 뒤 통째로 덮어쓰고** 있었다.
-- 잠금이 없으므로 같은 행에 시간 편집(이 RPC)과 상태 변경(클라)이 겹치면 뒤에 끝난 쪽이
-- 앞의 항목을 지운다 — 이력 jsonb Lost Update 의 4번째 경로다(#424·#402·#436 이 앞의 셋).
--
-- 역할 축의 서버 그릇은 **이미 있다**(staffRole/customRole 키 + role_change_history append).
-- 그래서 서버 신규 작업은 status 축 하나뿐이고, 클라 `updateRoleWithTransaction` 체인은
-- RPC 재구현이 아니라 **삭제**가 정답이다(호출부 0건의 죽은 코드 — 감사 finding-04).
--
-- ------------------------------------------------------------
-- 🔑 이 마이그가 만들어진 방식 — 전사 0회
-- ------------------------------------------------------------
-- 직전 정의(20260808120000)의 **파일을 그대로 복사**(md5 동일 확인)한 뒤 status 축만
-- 얹었다. 사람이나 에이전트가 700줄을 옮겨 적으면 주석 축약·공백 차이로 정본이 갈리고,
-- 동작이 같아 테스트로도 안 잡힌다(pitfall_prod_migration_comment_abridged_md5).
--
-- ------------------------------------------------------------
-- 🔴 순서 강제 — 이 파일이 prod 에 반영되기 **전에** 클라를 내보내지 마라
-- ------------------------------------------------------------
-- 패치 키 화이트리스트는 fail-closed 다. 신클라가 구서버에 `status` 를 보내면
-- `INVALID_INPUT: 알 수 없는 수정 항목입니다: status` 로 **상태 변경이 전면 파손**된다.
-- #441 이 정확히 이 형태였다(클라 선행 → 42703 → ops 전면 파손). 그래서 서버가 먼저다.
-- 역방향은 안전하다 — 구클라는 `status` 키를 보낼 코드 자체가 없어 기존 10키 경로가
-- 바이트 하나 안 바뀐다. 즉 이 마이그는 **단독 배포 가능**하고 되돌릴 필요가 없다.
--
-- 🔴 **직접 PATCH 차단 트리거는 이 파일에 없다** — 계측(testgap-01) 가동 확인 후로 보류.
--    계측 없이 넣으면 구클라 파손을 감지할 방법이 없다(감사 §6-9).
--
-- 파리티: CREATE OR REPLACE 라 함수 수 불변 (208 / 110). 정책 변경 0.
-- 회귀 고정: supabase/tests/work_log_slot_attendance_rpc.test.sql (기존 파일 **말미**에 추가)
-- ============================================================
...
DECLARE
  ...
  -- 상태 **명시 지정**(status 패치). 파생과 반드시 구분해야 한다:
  --   · 파생은 실적 시각에서 checked_out 까지만 만든다(아래 4 블록) — `completed` 는 도달 불가
  --   · 명시는 4종 전부를 만들 수 있고, 시각을 **역으로** 파생시킨다(클라 buildStatusTimestampPatch)
  v_set_status_explicit boolean := false;
  v_req_status          work_log_status;
  v_status_key_cnt      int;
...
BEGIN
  ...
  WHERE k NOT IN ('startTime', 'timeUndecided', 'staffRole', 'customRole', 'color', 'memo',
                  'editedBy', 'checkIn', 'checkOut', 'reason', 'status')
  LIMIT 1;
  ...
  -- 상태 명시 지정. 🔴 **허용 값은 근태 생애주기 4종뿐**이다.
  --   `no_show`·`cancelled` 를 받지 않는 이유: 둘은 동반 컬럼(no_show_reason/no_show_at)과
  --   복귀 규칙을 갖는 별도 도메인이고, 아래 파생 블록도 그 둘을 명시적으로 배제한다
  --   ("시간 수정이 노쇼를 조용히 checked_out 으로 뒤집으면 없던 유급 근무가 생긴다").
  --   여기서 열어 주면 no_show_reason 이 남은 채 status 만 바뀌는 모순 행이 만들어진다.
  --   노쇼 경로는 markAsNoShow/cancelNoShow 가 정본이다.
  IF p_patch ? 'status' THEN
    IF jsonb_typeof(p_patch->'status') <> 'string'
       OR NOT ((p_patch->>'status') IN ('scheduled', 'checked_in', 'checked_out', 'completed')) THEN
      RAISE EXCEPTION 'INVALID_INPUT: 알 수 없는 근무 상태입니다';
    END IF;
    v_req_status := (p_patch->>'status')::work_log_status;
    v_set_status_explicit := true;
  END IF;
  ...
  -- 🔴 불변식 — `status` 는 `reason`·`editedBy` 와만 동반한다.
  --    status 는 시각을 **역으로 파생**시키므로 checkIn/checkOut 과 같이 오면 어느 쪽이
  --    이기는지 계약에 없다. startTime/staffRole/color/memo 는 축이 겹치지는 않지만,
  --    "한 번에 한 의도" 를 깨면 이력 항목 하나에 서로 다른 사유가 섞인다.
  --    호출자는 두 번 부르면 된다(각각 이력이 남는 것이 오히려 정확하다).
  IF v_set_status_explicit THEN
    SELECT count(*)::int INTO v_status_key_cnt
    FROM jsonb_object_keys(p_patch) k
    WHERE k NOT IN ('status', 'reason', 'editedBy');
    IF v_status_key_cnt > 0 THEN
      RAISE EXCEPTION 'INVALID_INPUT: 근무 상태는 다른 항목과 함께 변경할 수 없습니다';
    END IF;
  END IF;
  ...
  -- ── 3-C) status 명시 지정 — 가드 + 시각 역파생 ──────────────
  IF v_set_status_explicit THEN
    -- 🔴 노쇼·취소 행은 이 경로로 되돌리지 않는다. 상태만 바꾸면 no_show_reason/no_show_at
    --    이 남은 **모순 행**이 된다(복귀는 잔여 시각에서 상태를 재구성해야 하고, 그건
    --    cancelNoShow 가 하는 일이다). 화면도 4종에서만 상태 시트를 연다.
    IF v_wl.status IN ('no_show', 'cancelled') THEN
      RAISE EXCEPTION 'INVALID_INPUT: 노쇼·취소된 근무는 이 경로로 상태를 바꿀 수 없습니다';
    END IF;

    -- 🔴 정산 완료건은 **시각 변경 여부와 무관하게** 거부한다(fail-closed).
    --    위 3-B 가 기존 키에 잠금을 걸지 않은 이유는 "구 빌드가 즉사한다" 였다.
    --    `status` 는 신설 키라 **구 빌드가 보낼 수 없다** — 처음부터 fail-closed 로 둘 수 있는
    --    유일한 시점이다. 그리고 status 는 서버 정산 게이트의 **판정축** 이므로
    --    (SettlementRepository 가 status 로 정산 가능 여부를 본다) 정산 후 뒤집으면
    --    정산의 사실 근거가 사후에 바뀐다. 클라 cancelNoShow 도 이미 같은 판정을 한다.
    IF v_wl.payroll_status = 'completed' THEN
      RAISE EXCEPTION 'ALREADY_SETTLED: 정산이 완료된 근무는 상태를 변경할 수 없습니다';
    END IF;

    -- 시각 역파생 — 클라 `buildStatusTimestampPatch` 를 그대로 옮긴 것이다.
    --   서버 정산 게이트는 status 로, 화면 게이트는 타임스탬프로 판정하므로 둘을 함께
    --   맞추지 않으면 "버튼은 눌리는데 서버가 영구 거부" 가 된다. 기존 값 우선(COALESCE)도 동일.
    IF v_req_status = 'scheduled' THEN
      v_new_check_in  := NULL;
      v_new_check_out := NULL;
    ELSIF v_req_status = 'checked_in' THEN
      v_new_check_in  := COALESCE(v_wl.check_in_ts, v_now);
      v_new_check_out := NULL;
    ELSE  -- checked_out / completed
      v_new_check_in  := COALESCE(v_wl.check_in_ts, v_now);
      v_new_check_out := COALESCE(v_wl.check_out_ts, v_now);
    END IF;

    -- 🔑 **실제로 값이 바뀌는 축만** 세운다 — 클라 `buildManualStatusAudit` 의 계약이다
    --    (안 바뀐 축까지 이력에 실으면 알림 본문에 무의미한 '종료  -> ' 가 붙는다).
    --    부수효과로 checked_out→completed 처럼 시각 무변경 전이는 이력이 남지 않는다 —
    --    이것도 클라와 동일한 기존 동작이다(변화 없으면 audit=null).
    v_set_check_in  := v_new_check_in  IS DISTINCT FROM v_wl.check_in_ts;
    v_set_check_out := v_new_check_out IS DISTINCT FROM v_wl.check_out_ts;
    v_touch_attendance := v_set_check_in OR v_set_check_out;

    -- status 는 요청값 그대로 쓴다. 아래 4) 의 파생은 checked_out 까지만 만들 수 있어
    -- `completed` 로 가는 경로가 없다 — 그래서 파생을 우회하고 여기서 확정한다.
    v_set_status := true;
    v_new_status := v_req_status;
  END IF;
  ...
  IF v_touch_attendance THEN
    ...
    -- 시간 수정이 노쇼를 조용히 checked_out 으로 뒤집으면 없던 유급 근무가 생긴다.
    -- 🔑 `status` 를 명시로 받았으면 파생하지 않는다(3-C 에서 이미 확정했다). 파생이 이기면
    --    '근무 완료 처리' 요청이 조용히 checked_out 으로 강등된다.
    IF NOT v_set_status_explicit
       AND v_wl.status IN ('scheduled', 'checked_in', 'checked_out', 'completed') THEN
  ...
  UPDATE public.work_logs SET
    ...
    end_time_source = CASE
                        -- 🔴 퇴근을 **지우면** 출처도 지운다. 예전엔 이 갈래가 없어
                        --    `checkOut: null` 패치에도 'manual' 이 박혔다 — 퇴근 시각이
                        --    없는데 "사람이 입력함" 이라는 모순된 행이 남고, 나중에 QR 로
                        --    다시 찍혀도 화면이 근거 없이 수기 기록처럼 읽힌다.
                        --    형제 경로(ConfirmedStaffRepository.ts:552)는 이 분기를 갖고 있었다.
                        WHEN v_set_check_out AND v_new_check_out IS NULL THEN NULL
                        WHEN v_set_check_out THEN 'manual'
                        ELSE end_time_source END,
...
COMMENT ON FUNCTION public.update_work_log_slot(uuid, jsonb) IS
  ...
  'custom_role 을 정리한다. status 키는 근태 생애주기 4종(scheduled/checked_in/checked_out/'
  'completed)만 받아 시각을 역파생하고(기존 값 우선) 실제로 바뀐 축만 이력에 남긴다 — '
  'reason·editedBy 와만 동반 가능하고, 노쇼·취소 행에는 쓸 수 없으며, 정산 완료건은 '
  '시각 변경 여부와 무관하게 거부한다(신설 키라 구 빌드 호환 부담이 없다). '
  '정산 완료건은 그 밖에는 실적 수정만 거부한다(역할 축은 잠기지 않는다). '
