-- ============================================================
-- update_work_log_slot 확장 — 커스텀 역할명(customRole) 키를 연다
--
-- 근거: 근무 시간 편집 통일 최종 리뷰가 찾은 **기능 소실**의 서버측 복구(Task 10).
--       클라(칩 UI·페이로드·진입점)는 Task 11 이 잇는다.
--
-- ── 무엇이 사라졌나 ─────────────────────────────────────────
...
COMMENT ON FUNCTION public.update_work_log_slot(uuid, jsonb) IS
  ...
  '지정할 수 있다(표준 역할과 동시 지정·other 아닌 행의 단독 지정은 INVALID_INPUT). '
