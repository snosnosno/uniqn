-- ============================================================
-- 퇴근 시각을 지우면 end_time_source 도 지운다 — 머지 리뷰 MEDIUM 4
--
-- update_work_log_slot 이 퇴근 축을 건드릴 때마다 end_time_source 를 'manual' 로 박았다.
-- 그런데 "건드린다"에는 **지우는 것**도 포함된다. `checkOut: null` 패치가 오면
-- check_out_ts 는 NULL 이 되는데 end_time_source 는 'manual' 로 남아,
-- **퇴근 시각이 없는데 "사람이 입력함" 이라고 주장하는 행**이 생긴다.
--
-- 형제 경로는 이 분기를 갖고 있었다 — ConfirmedStaffRepository.ts:552 가
-- `check_out_ts === null ? null : 'manual'` 로 갈랐다. 서버로 이관하면서 그 갈래만 누락됐다.
--
-- 피해는 조용하다: 지운 뒤 QR 로 다시 찍으면 트리거가 'qr' 로 덮으므로 자연 복구되지만,
-- 다시 안 찍힌 행은 계속 수기 기록처럼 읽힌다(화면의 "QR 기록" 배지 판정이 이 컬럼이다).
--
-- ------------------------------------------------------------
-- 🔑 이 마이그가 만들어진 방식 — 전사 0회
-- ------------------------------------------------------------
-- 직전 정의(20260807180000)의 **파일을 그대로 복사**한 뒤 end_time_source 한 줄만
-- 치환했다. 사람이나 에이전트가 680줄을 옮겨 적으면 주석 축약·공백 차이로 정본이 갈리고,
-- 동작이 같아 테스트로도 안 잡힌다(pitfall_prod_migration_comment_abridged_md5).
-- diff 실측: 추가 8줄 / 삭제 1줄 — 그 외 바이트 무변경.
--
-- 🔴 **적용 순서**: 20260807180000(퇴근≥출근 가드, prod 미적용) → **그 다음** 이 파일.
--    이 파일은 그 가드를 포함한 본문을 담고 있으므로 단독 적용해도 결과는 같지만,
--    schema_migrations 기록이 어긋나면 나중에 "무엇이 적용됐나"를 읽을 수 없다.
--
-- 파리티: CREATE OR REPLACE 라 함수 수 불변 (201 / 111).
-- 회귀 고정: supabase/tests/work_log_slot_attendance_rpc.test.sql (기존 파일에 케이스 추가)
-- ============================================================
