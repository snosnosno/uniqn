-- ops 배정 2종 후속: ops_reseat_participants 비-uuid 선검증(신뢰경계 방어, golden #6)
-- 근본원인: 20260630160000 의 `(e->>'participant_id')::uuid`/`(e->>'seat_id')::uuid` 캐스트가
--   비-uuid 문자열에서 raw 22P02('invalid input syntax for type uuid')를 던져, 친절
--   SEAT_ASSIGNMENT_INVALID(P0001) 경로를 우회했다. set_prize(20260630120100) 의 rank/amount
--   정규식 선검증과 동일 클래스.
-- 수정: 20260630160000 의 live 정의를 베이스로 CREATE OR REPLACE — step 3a 선검증 블록만 추가.
--   나머지 본문(락순서·가드·비우기/앉히기·이벤트) 전부 보존. 시그니처 불변 → 기존 GRANT/REVOKE 유지
--   (anon EXECUTE REVOKE·authenticated GRANT 는 CREATE OR REPLACE 가 ACL 보존, grants 마이그 불요).
