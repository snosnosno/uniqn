...
--      🔑 **부분 이동**으로 검사하는 이유: 전원을 옮기면 정원이 0 이 되는데, 가드가
--         `IF v_capacity > 0 AND ...` 라 0 을 "정원 미상"으로 보고 통째로 건너뛴다
--         (20260803120000:215). 그 상태로는 정원 이동의 효과를 증명할 수 없다.
...
-- 26. 정원이 0 이 되면 역할 항목을 남기지 않는다. floor 1명(정원 1)을 전부 옮긴다.
DO $$
BEGIN
...
SELECT is(
  (SELECT COALESCE(public._posting_schedule_role_count(schedule, '2026-09-05', '18:00', 'floor')::text, 'gone')
   ...
   FROM public.job_postings WHERE id = (current_setting('ups.jp_id'))::uuid),
  'gone|1|1',
  '정원이 0 이 된 역할은 지우되 같은 슬롯의 다른 역할과 슬롯 자체는 보존한다');
