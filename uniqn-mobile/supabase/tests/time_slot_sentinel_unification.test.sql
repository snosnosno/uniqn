...
BEGIN
  ...
  FROM unnest(ARRAY[v_s_neg, v_s_tba, v_s_range, v_s_pad, v_s_direct,
  ...
  UPDATE public.users SET is_active = true, identity_verified = true, name = '__r0_fixture'
   WHERE id = ANY (ARRAY[v_owner, v_s_neg, v_s_tba, v_s_range, v_s_pad, v_s_direct,
...
SELECT is(
  (SELECT string_agg(
            CASE wl.staff_id::text
              ...
              ELSE 'direct=' END
            || COALESCE(wl.time_slot, '<NULL>'), ' ' ORDER BY wl.created_at)
