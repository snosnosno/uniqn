...
--    클라가 '미정'/NULL 을 보내면 두 키가 갈리고, 정원 조회가 0행 → `v_capacity` 가
--    "정원 미상"이 되어 가드가 **통째로 건너뛰고 무제한 초과 배정**이 열린다.
--    (당시 표현은 `v_capacity`=0 + `IF v_capacity > 0`. 20260804140000 이 0 을 "자리 없음=거부"로
--     바꾸면서 우회는 `v_capacity IS NULL` 하나만 남았다 — 키가 갈리면 여전히 이 길로 빠진다.)
