...
- **잔재 정리**: 2026-06-24 전수감사 — 앱코드·의존성·prod DB 0 확인, 고아 Edge Function `revenuecat-webhook` 삭제, 죽은 `is_featured` SELECT 제거(PR #201).
- **아카이브 사유**: 위 구현 문서들은 더 이상 유효하지 않으므로 보존용으로 이관(2026-06-24).

> 향후 수익 방향(딜러 임금 정산-레일 / 노쇼 보증 등)은 별개 사업으로 **보류** 중이며 본 문서들과 무관합니다.
