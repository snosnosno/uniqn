...
export async function getCompletedByOwnerId(ownerId: string): Promise<WorkLog[]> {
  try {
    ...
    logger.info('구인자별 완료된 근무 기록 조회 완료', {
      ownerId,
      count: items.length,
    });
...
function getWorkLogRecencyMs(workLog: WorkLog): number {
  const updatedAt = TimeNormalizer.parseTime(workLog.updatedAt);
  if (updatedAt) {
    return updatedAt.getTime();
  }

  const createdAt = TimeNormalizer.parseTime(workLog.createdAt);
  if (createdAt) {
...
function getActiveFixedWorkLogStatusRank(workLog: WorkLog): number {
  if (workLog.status === STATUS.WORK_LOG.CHECKED_IN) {
    return 0;
  }

  if (workLog.status === STATUS.WORK_LOG.SCHEDULED) {
    return 1;
  }
...
export async function findActiveFixedByJobPostingStaff(
  jobPostingId: string,
  staffId: string
): Promise<WorkLog | null> {
  try {
    logger.info('fixed QR 가능 WorkLog 조회', { jobPostingId, staffId });
    ...
    const q = new QueryBuilder(workLogsRef)
      .whereEqual(FIELDS.WORK_LOG.jobPostingId, jobPostingId)
      .whereEqual(FIELDS.WORK_LOG.staffId, staffId)
      .whereEqual('isFixedPosting', true)
      .build();
    ...
    for (const docSnapshot of snapshot.docs) {
      ...
      if (
        workLog &&
        (workLog.status === STATUS.WORK_LOG.SCHEDULED ||
    ...
    if (items.length === 0) {
      logger.info('fixed QR 가능 WorkLog 없음', { jobPostingId, staffId });
      return null;
    }

    const [selected] = items.sort((left, right) => {
      const statusDiff =
        getActiveFixedWorkLogStatusRank(left) - getActiveFixedWorkLogStatusRank(right);
      if (statusDiff !== 0) {
        return statusDiff;
      }

      return getWorkLogRecencyMs(right) - getWorkLogRecencyMs(left);
    });

    logger.info('fixed QR 가능 WorkLog 조회 완료', {
      jobPostingId,
      staffId,
      found: true,
    ...
    return selected ? await hydrateWorkLogModificationHistory(selected, { force: true }) : null;
  } catch (error) {
    logger.error('fixed QR 가능 WorkLog 조회 실패', toError(error), {
      jobPostingId,
      staffId,
    });
    throw handleServiceError(error, {
      operation: 'fixed QR 가능 WorkLog 조회',
      component: 'WorkLogRepository',
