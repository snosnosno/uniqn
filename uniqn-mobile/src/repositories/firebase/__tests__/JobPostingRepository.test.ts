...
describe('FirebaseJobPostingRepository', () => {
  ...
  describe('updateWithTransaction', () => {
    ...
    it('should allow fixed posting updates when the canonical contract is satisfied', async () => {
      ...
      const result = await repository.updateWithTransaction(
        'job-1',
        {
          ...
          schedule: {
            ...
            daysPerWeek: 4,
            startTime: '19:00',
            roleRequirements: [{ role: 'dealer', count: 4, filled: 0 }],
          },
      ...
      expect(mockTransaction.set).toHaveBeenCalledTimes(1);
      const [, nextDocument] = mockTransaction.set.mock.calls[0];
      expect(nextDocument.schedule).toMatchObject({
        kind: 'fixed',
        daysPerWeek: 4,
        startTime: '19:00',
      });
