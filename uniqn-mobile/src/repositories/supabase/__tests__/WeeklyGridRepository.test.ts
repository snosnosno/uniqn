/**
 * WeeklyGridRepository — 그리드 읽기 RPC 래퍼 contract test
 *
 * 두 RPC 응답을 camelCase 로 투영(job_count→jobCount, work_log_id→workLogId 등)하는지와
 * 에러 전파를 검증한다. supabase.rpc 는 mock.
 */
import { SupabaseWeeklyGridRepository } from '../WeeklyGridRepository';
...
describe('WeeklyGridRepository', () => {
  const repo = new SupabaseWeeklyGridRepository();
