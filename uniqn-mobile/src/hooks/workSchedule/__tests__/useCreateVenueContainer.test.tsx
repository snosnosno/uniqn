 ...
 * (2) 성공 시 workSchedule 쿼리 일괄 invalidate, (3) workspaceId 부재 시 레포 미호출+에러.
...
jest.mock('@/repositories/workSchedule', () => ({ workScheduleRepository: {} }));
...
describe('useCreateVenueContainer', () => {
  ...
  it('성공 시 기존 컨테이너 목록에 새 컨테이너를 낙관적으로 시드 (N→N+1 자동선택 유지)', async () => {
    ...
    client.setQueryData(queryKeys.workSchedule.containers('ws-1'), [EXISTING_VENUE]);
    ...
    const cached = client.getQueryData<VenueContainer[]>(queryKeys.workSchedule.containers('ws-1'));
  ...
  it('낙관적 시드 멱등: 새 컨테이너가 이미 캐시에 있으면 중복 추가 안함', async () => {
    ...
    client.setQueryData(queryKeys.workSchedule.containers('ws-1'), [EXISTING_VENUE, NEW_VENUE]);
