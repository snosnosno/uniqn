 ...
 * 날짜 정규화(E5)는 레포/RPC 가 담당. onSuccess: weeklyGrid 관련 쿼리(summary/containers/daySlots)를
 * prefix(weeklyGrid.all)로 일괄 invalidate → useGridSummary(부족 N명)·컨테이너 softTargets 재조회.
 * 토스트/낙관 UI 는 호출부 책임(훅은 변이만).
 */
import { useMutation, useQueryClient } from '@tanstack/react-query';
import { queryKeys } from '@/lib/queryClient';
import { setVenueSoftTarget } from '@/services/weeklyGrid/gridWriteService';
...
export default useSetVenueSoftTarget;
