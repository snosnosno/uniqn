...
import { buildGridCells, type GridDayCell } from '@/domains/weeklyGrid';
...
export function useGridSummary(
  ...
  return useQuery<Record<string, GridDayCell>>({
    queryKey: venueId
      ? queryKeys.weeklyGrid.summary(venueId, range.from, range.to)
      : [...queryKeys.weeklyGrid.all, 'summary', 'none'],
    queryFn: async () => {
      const vid = venueId as string;
      const [rows, container] = await Promise.all([
        weeklyGridRepository.getVenueGridSummary(vid, range.from, range.to),
