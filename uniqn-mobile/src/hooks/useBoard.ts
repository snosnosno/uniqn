...
async function requireBoardAuthorIdentity(
  ...
  role: BoardAuthorRole | null | undefined,
  isAdmin = false
): Promise<{
  userId: string;
  isAdmin: boolean;
...
export function useBoardPosts(boardType: CommunicationBoardType, limitCount?: number) {
