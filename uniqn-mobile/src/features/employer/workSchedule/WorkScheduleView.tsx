/**
 * UNIQN Mobile — 근무표 본문 (운영처 월 캘린더 오버뷰)
 *
 ...
 *
 * 구인자 IA S3 — 같은 본문을 두 곳이 쓴다.
 * - `app/(employer)/work-schedule.tsx` (딥링크·알림 도착지): `renderHeader` 로 StackHeader 를 그린다.
 * - 내 공고 탭 [근무표] 세그먼트: 탭 헤더가 이미 있으므로 헤더 없이, `금액` 진입점을 본문에 둔다.
 * 플래그 판정은 호출부 책임이다 — 이 컴포넌트는 플래그 ON 에서만 마운트된다.
 */
...
/** 바깥에서 특정 날짜로 옮겨 달라는 요청. 같은 날짜를 다시 눌러도 반응하도록 요청 시각을 함께 받는다. */
export interface WorkScheduleFocusRequest {
  date: string;
  requestedAt: number;
}

export interface WorkScheduleViewProps {
  /**
   * 헤더 렌더러. `amountAction` 은 지점 소유자에게만 보이는 `금액` 버튼(없으면 undefined).
   * 넘기지 않으면 헤더 없이 그리고, `금액` 버튼은 본문 상단에 둔다(진입점을 잃지 않기 위해).
   */
  renderHeader?: (amountAction: ReactNode | undefined) => ReactNode;
  focusRequest?: WorkScheduleFocusRequest | null;
  /**
   * 스크롤 본문 하단 여백. 스택 화면은 기본값(24)이면 되지만, 탭 안에 들어가면 탭바가
   * 마지막 날짜 패널을 가리므로 호출부가 `useTabBarBottomPadding()` 값을 넘긴다.
   */
  contentBottomPadding?: number;
}

const DEFAULT_CONTENT_BOTTOM_PADDING = 24;
...
export function WorkScheduleView({
  ...
  const { isCreating: isCreatingWorkspace, retry: retryCreateWorkspace } =
    useEnsureDefaultWorkspace({
      isReady: !wsLoading && !wsError,
      isEmpty: workspaces.length === 0,
    });

  const containersQuery = useVenueContainers(activeWorkspace?.id, { enabled: true });
  ...
  const { isCreating: isAutoCreatingVenue } = useEnsureDefaultVenue({
    ...
    isReady: !wsLoading && containersQuery.isSuccess,
  ...
  const summaryQuery = useGridSummary(selectedVenueId, visibleMonth, { enabled: true });
  ...
  // 날짜로 옮길 때는 달도 함께 옮긴다 — 날짜만 바꾸면 월 동기화 useEffect 가 그 날짜를
  // 현재 달 안으로 도로 끌어와 무반응이 된다.
  const moveToDate = useCallback((dateKey: string) => {
  ...
  // 배지를 누르면 가장 오래된 미기록 날짜로 이동한다. 지난 달일 수 있다.
  const handleGoToMissingCheckout = useCallback(() => {
    if (!missingCheckouts.earliestDate) return;
    moveToDate(missingCheckouts.earliestDate);
  }, [missingCheckouts.earliestDate, moveToDate]);

  // 바깥(내 공고 탭 "오늘 한 줄")에서 온 이동 요청.
  useEffect(() => {
    if (!focusRequest) return;
    moveToDate(focusRequest.date);
  }, [focusRequest, moveToDate]);
  ...
  return (
    <View className="flex-1">
      {renderHeader ? renderHeader(amountAction) : null}
      ...
      {/* 헤더가 없는 탭 임베드에서는 `금액` 을 본문 오른쪽 위에 둔다 — 진입점을 잃지 않는다. */}
      {!renderHeader && amountAction ? (
        <View className="flex-row justify-end px-2">{amountAction}</View>
      ) : null}
