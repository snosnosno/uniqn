 ...
 *
 * @description 오늘 그룹 유무·출근(checkInTime 기준) 집계·노쇼 조건부 노출을 잠근다.
 *   구인자 IA S2 — 정산 대기 배지는 없앴다. 앱이 돈을 만지지 않으니 "대기"도 없다.
 */
...
describe('TodayOpsStrip', () => {
  it('오늘 그룹이 없으면 아무것도 렌더하지 않는다', () => {
    const { toJSON } = render(<TodayOpsStrip todayGroup={undefined} />);
  ...
  it('출근 집계는 status가 아니라 checkInTime 존재 기준이다', () => {
    ...
    const { getByText } = render(<TodayOpsStrip todayGroup={group} />);
    expect(getByText('출근 2/3')).toBeTruthy();
  });

  it('노쇼 0이면 노쇼 배지를 표시하지 않는다', () => {
    const group = makeTodayGroup([makeStaff()]);
    const { queryByText } = render(<TodayOpsStrip todayGroup={group} />);
  ...
  it('정산 대기를 말하지 않는다 — 지급 상태가 없다', () => {
    ...
    const { queryByText, UNSAFE_root } = render(<TodayOpsStrip todayGroup={group} />);

    expect(queryByText(/정산/)).toBeNull();
    // 요약 라벨(스크린리더)에서도 사라져야 한다.
    const summary = UNSAFE_root.findAll(
      (node: { props: Record<string, unknown> }) => node.props.accessibilityRole === 'summary'
    )[0];
    expect(String(summary.props.accessibilityLabel)).not.toContain('정산');
  });
});
