 ...
 * actualStartTime/checkInTime 등 다양한 시간 필드를 통일된 인터페이스로 정규화
 */
 ...
 * - checkInTime/actualStartTime 통합
 * - checkOutTime/actualEndTime 통합
 * - scheduledStartTime/scheduledEndTime 통합
 * - 근무 시간 계산
 */
export class TimeNormalizer {
   ...
   *
   * @description 우선순위:
   * - actualStartTime > checkInTime (실제 출근)
   * - actualEndTime > checkOutTime (실제 퇴근)
   *
  ...
  static normalize(input: TimeFieldsInput): NormalizedWorkTime {
    ...
    // 실제 시간 정규화 (actualStartTime 우선, 없으면 checkInTime)
    const actualStart =
      this.parseTime(input.actualStartTime) ??
      this.parseTime(input.checkInTime);

    const actualEnd =
      this.parseTime(input.actualEndTime) ??
      this.parseTime(input.checkOutTime);
