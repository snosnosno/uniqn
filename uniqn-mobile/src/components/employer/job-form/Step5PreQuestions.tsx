/**
 * UNIQN Mobile - 공고 작성 Step 5: 사전질문
 *
...
interface Step5PreQuestionsProps {
...
export function Step5PreQuestions({ data, onUpdate, onNext, onBack }: Step5PreQuestionsProps) {
  ...
  const handleAddQuestion = useCallback(() => {
    if (data.preQuestions.length >= 10) {
      setError('최대 10개까지 질문을 추가할 수 있습니다');
      return;
    }
    const newQuestion: PreQuestion = {
      id: generateId(),
  ...
  const validate = useCallback(() => {
    if (!data.usesPreQuestions) return true;
    ...
    if (hasEmptyOption) {
      setError('선택지 내용을 입력해주세요');
      return false;
    }

    setError('');
    return true;
  }, [data.usesPreQuestions, data.preQuestions]);
...
export default Step5PreQuestions;
