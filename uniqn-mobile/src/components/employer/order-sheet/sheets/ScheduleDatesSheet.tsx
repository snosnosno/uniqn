 ...
 * ⚠️ 문구 규칙: 축이 2개(지원자 선택 자유도 / 날짜별 조건 상이)인데 라디오는 1차원이라,
 * ①을 기준점으로 두고 ②③은 "기준에서 달라지는 것"만 말한다. ①의 hint 를 지원자 축만으로
 * 쓰면 ③과 구별이 안 되고(③도 날짜별 지원), 조건 축만으로 쓰면 ②와 구별이 안 된다 —
 * 그래서 ①만 두 축을 함께 말한다. 참고: ③으로 쪼개도 조건을 바꾸지 않으면 복원 시
 * 시그니처 병합으로 ①과 동일 그룹이 된다(mappers.ts:349-360) — ③은 결과가 아니라 편집 준비다.
  ...
  return [
    {
      ...
      label: '모든 날짜 같은 조건',
      badge: '기본',
      hint: '전 날짜 시간·역할이 같아요 · 지원자는 원하는 날짜만 골라 지원',
    },
    {
      mode: 'grouped',
      label: '연속된 날짜는 통째로',
    ...
    },
    {
      ...
      hint:
        count > 1
          ? `날짜마다 시간·역할·인원이 달라요 · 일정 카드가 ${count}개로 나뉘어요`
          : '날짜마다 시간·역할·인원이 다를 때 골라요',
    },
  ];
}
