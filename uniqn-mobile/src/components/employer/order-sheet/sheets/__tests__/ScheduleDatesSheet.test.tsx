...
describe('ScheduleDatesSheet — 세그먼트 클램프 (리뷰 M-1)', () => {
  ...
  it('②는 실제 연속 구간 날짜를, ③은 나뉘는 카드 수를 문구에 담는다', () => {
    ...
    // ①=기준점(두 축 모두 명시) / ②=지원 규칙만 다름 / ③=조건만 다름.
    // ① hint 를 한 축으로만 쓰면 ②(조건 축 누락) 또는 ③(지원 축 누락)과 구별이 사라진다.
    expect(getByText('전 날짜 시간·역할이 같아요 · 지원자는 원하는 날짜만 골라 지원')).toBeTruthy();
    expect(getByText('7/20~7/21 다 나올 사람만 지원해요 (하루만 지원 불가)')).toBeTruthy();
    expect(getByText('날짜마다 시간·역할·인원이 달라요 · 일정 카드가 3개로 나뉘어요')).toBeTruthy();
  });
