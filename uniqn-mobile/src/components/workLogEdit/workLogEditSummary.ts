...
/**
 * ⚠️ 출근 예정용 요약 함수는 **의도적으로 없다.** 예정 축은 `WorkTimeFields` 안에 실적과 한
 *    덩어리로 들어 있어 접히지 않으므로(설계 §3 ↔ Task 5 구조 불일치, Task 7 보고서 참조),
 *    요약을 소비할 자리가 없다. 예정이 접히는 구조로 바뀌면 그때 되살린다 —
 *    빈 상태 문구는 지금 `WorkTimeFields` 의 예정 행이 '미정'으로 직접 말하고 있다.
 */
