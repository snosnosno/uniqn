 ...
 *
 * ## v3.0 변경사항
 * - `role?: string`과 `roles?: string[]`를 `roleIds`로 통합
 * - 단일 역할도 배열로 표현: `roleIds: ['dealer']`
 * - 다중 역할: `roleIds: ['dealer', 'floor']`
 *
 * ## v3.1 변경사항
 * - `roleIds: string[]` → `roleIds: StaffRole[]` (타입 안전성 강화)
 *
 * @example
 * // 단일 날짜, 단일 역할
...
export interface Assignment {
  /**
   * 역할 ID 배열 (v3.0: role/roles 통합, v3.1: StaffRole 타입 적용)
   * - 단일 역할: ['dealer']
   * - 다중 역할: ['dealer', 'floor']
