/**
 * opsHubFlag — ops 라이브 운영 허브 진입 표면 원격 플래그 순수 파서
 *
 * app_config.ops_hub_enabled 의 value 는 `{"enabled": boolean}` 모양이다.
 ...
 */
export function parseOpsHubFlag(raw: unknown, fallback: boolean): boolean {
