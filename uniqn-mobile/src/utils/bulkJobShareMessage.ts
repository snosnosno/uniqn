 ...
 * 제목만 담은 최소 항목으로 degrade 한다(단일 공유와 동일 정책).
 */
export function buildBulkShareItem(
  ...
  try {
    ...
    void toError(error);
    return { title: fallbackTitle, location: '', scheduleLabel: '', roleLine: '', salary: '' };
  }
}

/**
 * 항목 1건의 본문 블록. 값이 없는 라인은 통째로 생략한다.
 * 개별 공고 링크는 넣지 않는다 — 본문에 링크가 여러 개면 카톡이 그중 하나만 스크랩해
 * 엉뚱한 공고의 미리보기 카드가 뜬다. 링크는 맨 끝 구인구직 탭 하나로 모은다.
 */
 ...
 *
 * 링크는 본문 맨 끝 **구인구직 탭 하나뿐**이다. 개별 공고 링크를 N개 넣으면
 * 카톡이 그중 하나만 스크랩해 목록 공유인데 특정 공고 카드가 뜨고, 메시지도 N줄 길어진다.
 *
 * 길이 상한을 넘기면 넘긴 항목부터 잘라내고 "외 N건" 안내를 붙인다.
 * 최소 1건은 항상 포함한다(상한이 아무리 작아도 빈 메시지를 보내지 않는다).
 *
 * @param listUrl 구인구직 탭 웹 URL
 */
export function composeBulkJobShareText(
  ...
  const footer = `\n\n👉 지원하기\n${listUrl}`;
  const omissionNote = (count: number) => `\n\n…외 ${count}건은 앱에서 확인해주세요`;
  // 생략 안내문의 최악 길이(전건 생략 가정) — 이걸 미리 빼두지 않으면 안내문 때문에
  // 결과가 상한을 넘는다.
  const noteReserve = omissionNote(items.length).length;

  const included: string[] = [];
  // 꼬리 링크는 반드시 살아남아야 하므로 상한 계산에 미리 포함한다.
  ...
  for (const [index, item] of items.entries()) {
    ...
    // 마지막 항목까지 들어가면 안내문이 붙지 않으므로 예약분이 필요 없다.
    const reserve = index === items.length - 1 ? 0 : noteReserve;

    if (included.length > 0 && projected + reserve > maxChars) {
      // 여기서 멈춘다 — 뒤쪽 짧은 항목을 끼워 넣으면 번호가 1, 2, 4처럼 튄다.
      break;
    }
  ...
  const tail = omitted > 0 ? omissionNote(omitted) : '';
 ...
 * @param listUrl   본문 맨 끝에 한 번 붙일 구인구직 탭 URL
...
export function buildBulkJobShareText(
  ...
  const items = sortJobsForBulkShare(jobs).map((job) =>
    buildBulkShareItem(job, filledFor?.(job.id))
  );
  return composeBulkJobShareText(items, listUrl, options);
}
