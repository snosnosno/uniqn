 ...
 *
 * 'unpaid'(미지급) 축은 구인자 IA S2b 에서 없앴다 — 앱은 돈을 보내지 않고, 사장이
 * `지급 완료` 를 누르는 흐름도 없어 모든 완료 근무가 영원히 "미지급" 으로 잡힌다.
 */
...
// `isUnpaidCompleted` / `countUnpaidSchedules`('미지급' 필터)는 구인자 IA S2b 에서 없앴다 —
// 사장이 `지급 완료` 를 누르는 흐름이 없으니 모든 완료 근무가 영원히 "미지급" 으로 잡힌다.
