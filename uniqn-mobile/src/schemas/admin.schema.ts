...
export const changeUserRoleSchema = z.object({
  ...
  reason: z.string().max(200, { message: '사유는 200자를 초과할 수 없습니다' }).optional(),
});
...
export const changeUserStatusSchema = z.object({
  ...
  reason: z.string().min(1, { message: '상태 변경 사유는 필수입니다' }).max(500, { message: '사유는 500자를 초과할 수 없습니다' }),
...
export const suspendUserSchema = z.object({
  userId: z.string().min(1, { message: '사용자 ID는 필수입니다' }),
  reason: z.string().min(10, { message: '정지 사유는 최소 10자 이상이어야 합니다' }).max(500),
  duration: z.number().min(1, { message: '정지 기간은 최소 1일 이상이어야 합니다' }).max(365, { message: '정지 기간은 365일을 초과할 수 없습니다' }),
});
...
export const createReportSchema = z.object({
  ...
  description: z.string().min(10, { message: '신고 내용은 최소 10자 이상이어야 합니다' }).max(1000, { message: '신고 내용은 1000자를 초과할 수 없습니다' }),
...
export const createAnnouncementSchema = z.object({
  ...
  content: z.string().min(1, { message: '내용은 필수입니다' }).max(5000, { message: '내용은 5000자를 초과할 수 없습니다' }),
