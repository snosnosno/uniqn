...
export const inquiryAttachmentSchema = z.object({
  ...
  size: z.number().max(5 * 1024 * 1024, { message: '파일 크기는 5MB를 초과할 수 없습니다' }).optional(),
});
...
export const createInquirySchema = z.object({
  ...
  attachments: z.array(inquiryAttachmentSchema).max(3, { message: '첨부파일은 최대 3개까지 가능합니다' }).optional(),
});
...
export const inquiryFilterSchema = z.object({
  ...
  category: z.enum(['all', 'general', 'technical', 'payment', 'account', 'report', 'other']).optional().default('all'),
});
