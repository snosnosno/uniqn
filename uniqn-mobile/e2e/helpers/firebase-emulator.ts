...
const FUNCTIONS_EMULATOR_URL = 'http://localhost:5001';
const STORAGE_EMULATOR_URL = 'http://localhost:9199';
...
export async function isFunctionsEmulatorRunning(): Promise<boolean> {
  try {
    const response = await fetch(`${FUNCTIONS_EMULATOR_URL}/`);
    return response.ok;
  } catch {
    return false;
  }
...
export async function isStorageEmulatorRunning(): Promise<boolean> {
  try {
    const response = await fetch(`${STORAGE_EMULATOR_URL}/`);
...
export async function clearAuthEmulator(projectId: string): Promise<void> {
  await fetch(`${AUTH_EMULATOR_URL}/emulator/v1/projects/${projectId}/accounts`, {
    method: 'DELETE',
  });
}
