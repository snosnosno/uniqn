import { test, expect } from '../../fixtures/base.fixture';

async function waitForAppBootstrap(page: Parameters<typeof test.beforeEach>[0]['page']) {
  await page.waitForLoadState('domcontentloaded');

  for (const text of ['앱 로딩 중...', '앱을 불러오는 중...']) {
    const loading = page.getByText(text);
    const isVisible = await loading.isVisible().catch(() => false);

    if (isVisible) {
      await loading.waitFor({ state: 'hidden', timeout: 30_000 }).catch(() => {});
    }
  }
}

test.describe('지원센터', () => {
  test.beforeEach(async ({ page }) => {
    await page.goto('/support', { waitUntil: 'domcontentloaded' });
    await waitForAppBootstrap(page);
    await expect(page.getByText('자주 묻는 질문', { exact: true })).toBeVisible({
      timeout: 20_000,
    });
  });

  test('지원센터 메인에 3개 메뉴가 보인다', async ({ page }) => {
    await expect(page.getByText('자주 묻는 질문', { exact: true })).toBeVisible();
    await expect(page.getByText('1:1 문의하기')).toBeVisible();
    await expect(page.getByText('문의 내역')).toBeVisible();
  });

  test('운영시간 안내가 보인다', async ({ page }) => {
    await expect(page.getByText('고객센터 운영시간')).toBeVisible();
    await expect(page.getByText(/평일 09:00/)).toBeVisible();
  });

  test('FAQ 메뉴는 텍스트 클릭으로도 이동한다', async ({ page }) => {
    await page.getByText('자주 묻는 질문', { exact: true }).click();
    await page.waitForURL(/\/support\/faq$/, { timeout: 5_000 });
  });

  test('FAQ 메뉴는 카드 클릭으로도 이동한다', async ({ page }) => {
    await page.getByTestId('support-menu-faq').click();
    await page.waitForURL(/\/support\/faq$/, { timeout: 5_000 });
  });
});

test.describe('FAQ', () => {
  test.beforeEach(async ({ page }) => {
    await page.goto('/support/faq', { waitUntil: 'domcontentloaded' });
    await waitForAppBootstrap(page);
    await expect(page.getByRole('heading', { name: '자주 묻는 질문' })).toBeVisible({
      timeout: 20_000,
    });
  });

  test('FAQ 화면은 카테고리와 CTA를 렌더링한다', async ({ page }) => {
    await expect(page.getByText('UNIQN은 어떤 서비스인가요?')).toBeVisible();
    await expect(page.getByText('원하는 답변을 찾지 못하셨나요?')).toBeVisible();
    await expect(page.getByText('1:1 문의하기')).toBeVisible();
  });

  test('FAQ 하단 CTA가 있으면 1:1 문의 버튼이 같이 보인다', async ({ page }) => {
    const ctaText = page.getByText('원하는 답변을 찾지 못하셨나요?');
    const ctaButton = page.getByText('1:1 문의하기');

    const hasCta = await ctaText.isVisible().catch(() => false);
    if (hasCta) {
      await expect(ctaButton).toBeVisible();
    }
  });
});

test.describe('문의 내역', () => {
  test('문의 내역 화면은 목록 또는 빈 상태를 렌더링한다', async ({ page }) => {
    await page.goto('/support/my-inquiries', { waitUntil: 'domcontentloaded' });
    await waitForAppBootstrap(page);

    const content = page.getByText(/문의 내역이 없습니다|문의 내역/);
    const isVisible = await content.isVisible().catch(() => false);
    expect(typeof isVisible).toBe('boolean');
  });
});

test.describe('1:1 문의 작성', () => {
  test.beforeEach(async ({ page }) => {
    await page.goto('/support/create-inquiry', { waitUntil: 'domcontentloaded' });
    await waitForAppBootstrap(page);
    await expect(page.getByText('문의 유형').first()).toBeVisible({ timeout: 20_000 });
  });

  test('문의 작성 화면에 필수 입력 요소가 보인다', async ({ page }) => {
    await expect(page.getByText('문의 유형').first()).toBeVisible();
    await expect(page.getByPlaceholder('문의 제목을 입력해주세요')).toBeVisible();
    await expect(page.getByPlaceholder('문의 내용을 상세히 입력해주세요')).toBeVisible();
    await expect(page.getByText('문의하기', { exact: true })).toBeVisible();
  });

  test('문의 내용 입력 시 글자 수 카운터가 보인다', async ({ page }) => {
    const messageInput = page.getByPlaceholder('문의 내용을 상세히 입력해주세요');
    await messageInput.fill(
      '테스트 문의 내용입니다. 넉넉한 글자 수 카운터 표시를 확인하기 위한 테스트 입력입니다.'
    );

    await expect(page.getByText(/\d+\/2000/)).toBeVisible();
  });
});
