...
test.describe('Admin 대시보드', () => {
  ...
  test('대시보드 메인 페이지 렌더링 → 제목 및 메뉴 카드 표시', async () => {
    // 앱 초기화 후 대시보드 제목 확인 (넉넉한 타임아웃)
    await expect(dashboard.title).toBeVisible({ timeout: 30_000 });
  ...
  test('통계 페이지 주요 지표 표시', async () => {
    ...
    const statsOrLoading = dashboard.statsSection
      ...
      .or(dashboard.page.getByText(/서비스 통계/))
      .or(dashboard.errorMessage);
    await expect(statsOrLoading).toBeVisible({ timeout: 30_000 });
  ...
  test('통계 페이지 7일 트렌드 차트 렌더링', async () => {
    ...
    // 통계 페이지 로드 확인
    const pageLoaded = dashboard.page.getByText(/서비스 통계/)
      .or(dashboard.trendSection)
      .or(dashboard.page.getByText(/통계 데이터를 불러오는 중/))
      .or(dashboard.errorMessage);
    await expect(pageLoaded).toBeVisible({ timeout: 30_000 });
