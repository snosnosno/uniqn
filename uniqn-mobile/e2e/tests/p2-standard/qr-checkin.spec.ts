...
test.describe('QR 출퇴근 스캔 진입 동선', () => {
  ...
  // ⚠️ 격리 (2026-07-21) — 이 테스트는 도입(#286) 이후 **한 번도 green 인 적이 없다**.
  // 회귀가 아니라 미완성 테스트이며, red 로 머지되어 #286·#287·#290·#291 네 PR 연속으로
  // E2E 전체를 red 로 만들었다. 그 사이 review-system 실패가 묻혔다 — 신호를 죽이는 쪽이
  // 이 한 건을 방치하는 것보다 비싸서 빚을 장부에 올리고 격리한다.
  //
  // 조사로 기각한 원인 (2026-07-21):
  // - 웹 번들 라우트 누락 → 기각. `(app)/scan` 5회, '출퇴근 QR 스캔' 2회, '카메라 권한' 4회
  //   모두 메인 번들에 존재 (한글은 \uXXXX 이스케이프라 raw grep 으로는 0 이 나온다)
  // - 튜토리얼 오버레이가 가림 → 기각. 실패 스냅샷에 오버레이 없음, 키·버전·hashUID 일치
  // - jsQR lazy 청크 로드 실패 → 기각. 스캐너 마운트 전 단계에서 멈춘다
  // - 버튼/라우트 코드 결함 → 기각. Pressable onPress → router.push, 호출부 4곳 표기 일관
  // - `/scan` 이 웹에서 마운트 안 됨 → 기각. 미인증 접근 시 auth guard 리다이렉트가 정상
  //   동작하고 콘솔 에러 0건, "웹 카메라 중지됨" 로그로 스캐너 마운트까지 확인
  //
  // 남은 가설: E2E storageState 인증 상태에서만 재현되는 하네스 문제.
  // 재개 조건 = 로컬 재현 수단(e2e/.env.test) 확보 후 트레이스로 클릭→네비게이션 관찰.
  test.skip('헤더 QR 버튼 1탭이면 스캔 화면이 열린다', async ({ page }) => {
