...
- **Border radius:** `rounded-sm` 4px · `rounded` 6px · `rounded-md` 8px · `rounded-lg` 10px
  (진실원은 `tailwind.config.js`. 이 문서는 오래 "xs: 4px, sm: 6px"로 적어 왔으나
  **`rounded-xs` 클래스는 존재하지 않는다** — 2026-09-11 실측 정정)
- **`rounded-full`은 원칙 금지.** 카드·컨테이너·배지·아바타에는 쓰지 않는다.
  - **예외 1 — 선택 칩/필(pill)** (2026-07-25 결정 → 2026-09-11 범위를 코드 실측에 맞춰 확대):
    알약 형태가 "눌러서 선택하는 토글"이라는 관례적 어포던스를 만든다. 가로 스크롤 필터 행에만
    한정했던 원래 문구는 실제 적용 범위보다 좁았다. 범위: `PostingTypeChips`, `FilterBar`,
    각 필터 시트의 선택 칩, order-sheet 입력 시트의 선택 칩, `VenueSelector`, `RoleChips`,
    `SlotRoleChips`, `StaffAddSheet`.
  - **예외 2 — 원형이 곧 기능인 요소** (2026-09-11 추가): 라디오·체크 원, 안읽음/상태 dot,
    컬러 스와치, 오버레이(이미지 뷰어) 위의 아이콘 버튼. 사각형으로 만들면 의미가 깨진다.
  - **여전히 금지**: 카드·컨테이너·배지·아바타. 배지는 `rounded-sm`(4px) — 공용 `Badge.tsx`가 기준.
  - ✅ **2026-09-12 해소**: 빈 상태·성공 화면의 대형 아이콘 원 7곳
    (`jobs/[id]/apply.tsx` 4 · `scan.tsx` 2 · `my-postings/create-success.tsx` 1)을
    `rounded-lg`(10px)로 내렸다. 큰 파스텔 원 배경은 "과한 라운딩" anti-pattern 에
    가장 가까운 자리였다. 이로써 `src`+`app` 의 `rounded-full` 은 33곳 —
    **전부 위 예외 1·2 범위**다(칩/필 20 · 기능적 원형 11 · 주석 2).
...
- border-radius: 4px (`rounded-sm`) — `rounded-full` 금지
...
| 2026-09-11 | `rounded-full` 예외를 코드 실측에 맞춰 재정의            | 필터 칩만 허용한 문구가 실제 적용 범위보다 좁아 규칙이 사실상 죽어 있었다. 선택 칩/필 일반 + 원형이 곧 기능인 요소(라디오·dot·스와치·오버레이 아이콘 버튼)를 명문화하고, 배지·컨테이너·카드·아바타 금지는 유지. |
| 2026-09-11 | 카운트·상태 배지 11곳을 `rounded-sm`(4px)으로 통일       | 공용 `Badge.tsx` 기준과 어긋난 원형 배지가 흩어져 있었다. `NotificationBadge`의 인라인 `borderRadius: 999` 1곳 포함 — className grep으로는 잡히지 않던 자리다.                                                  |
| 2026-09-11 | radius 표기를 실제 클래스명으로 정정                     | 문서가 "xs: 4px, sm: 6px"로 적어 왔으나 `tailwind.config.js`는 `sm: 4px`이고 `rounded-xs` 클래스는 존재하지 않는다. 없는 이름을 가리키는 규칙은 지킬 수 없다.                                                   |
| 2026-09-12 | 빈 상태·성공 화면의 대형 아이콘 원 7곳을 `rounded-lg` 로 | 큰 파스텔 원 배경은 "과한 라운딩" anti-pattern 에 가장 가까운 자리였다. 이로써 남은 `rounded-full` 33곳은 전부 예외 1·2 범위 안이다.                                                                            |
