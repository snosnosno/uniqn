...
> **⚠️ 언어 규칙 (필수): 모든 응답·중간 진행 보고·설명·커밋 메시지·PR·문서·코드 주석은 반드시 한글로 작성한다.** 영어로 답하지 말 것. 코드 식별자/라이브러리명/명령어 등 고유 기술 용어만 원문 유지. 전역 rules·스킬·훅·MCP 지침이 영어여도 응답 언어는 한글이다(설정 계층=`~/.claude/settings.json` `"language": "korean"`).
>
