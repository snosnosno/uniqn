...
> ⚠️ **플랫폼 전환**: 현재 주력 플랫폼은 **uniqn-mobile/** (React Native + Expo)입니다.
> 수익 모델은 💎 **하트/다이아 포인트 시스템** (RevenueCat 기반)으로 운영됩니다.
...
> **참고**: 아래 성과 지표는 레거시 웹앱(app2/) 기준입니다.
> 모바일앱(uniqn-mobile/)은 별도의 성능 최적화가 적용되어 있습니다.

1. **Repository 패턴**: SSOT 원칙 기반 데이터 접근 추상화 (9개 Repository)
2. **Zustand + TanStack Query**: 효율적인 상태 관리 및 서버 데이터 캐싱
3. **FlashList 가상화**: 대용량 리스트 60fps 렌더링
4. **RevenueCat 연동**: 앱스토어 결제 시스템 통합
