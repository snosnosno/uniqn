...
> 작성: 2026-06-30 · 갱신: 2026-07-02(배정 2종 #220 출하 후). 목적 = ①지금까지 출하된 모든 슬라이스(1a~1d + 배정 2종) 신선 컨텍스트 **재점검·정합 확인** → ②**남은 슬라이스(1f 잔여 상금[우선]·1e 스태프) 재매핑 후 설계**. 코딩 전 브레인스토밍·계획 게이트.
>
> ✅ **1d(#218) + 배정 2종(#220)은 출하 완료**(prod 마이그 적용·advisor ERROR0). 배정 2종=랜덤·칩드래프트 전원 재배치(좌석-우선 락·전원 비우기→앉히기·피처테이블 소스보호, 적대검증 11결함 머지전 하드닝, jest4557·pgTAP390·master `685e4e1f8`). 따라서 이 세션은 **출하 여부 결정 불요 — 남은 슬라이스 = (다)1f 잔여 상금[우선·소규모, 1d ops_prizes 위] → (나)1e 스태프[⚠️prod `staff_management_direct_add` 실측 후] 설계로 바로 진행**한다. 방법론=배정 2종 PR #220 패턴 재사용(정찰WF→브레인스토밍→계획→적대검증WF→SDD 3배치→검증→prod게이트).
