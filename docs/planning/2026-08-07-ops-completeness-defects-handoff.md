...
> 선행 세션 산출물: 브랜치 **2개**(`fix/ops-web-origin-20260807` 머지분 · `fix/ops-native-deeplink-20260807` 대기분) — 아래 §0 참조.
...
원본은 한 브랜치였으나 **전달 경로가 갈려 둘로 분리했다.** version bump 는 머지하는 순간
이후의 모든 `eas update` 를 runtime 1.0.6 으로 보내 **현재 설치된 1.0.5 기기에 아무 OTA 도
닿지 않게** 만든다. 대기 중인 다른 수정까지 스토어 심사를 기다리게 되므로, OTA 로 전달 가능한
분을 먼저 떼어냈다.

| 브랜치 | 내용 | 전달 경로 | 상태 |
|---|---|---|---|
| `fix/ops-web-origin-20260807` | ①ops 공개 링크 폴백을 죽은 도메인(`ops.uniqn.app`)에서 `APP_WEB_ORIGIN` 으로. `deploy:ops` 별칭 제거, 회귀 테스트 6종 신설 ②AASA 에 `/jobs`(정확 일치) 추가 — 공고 목록 공유 링크가 iOS 에서만 앱으로 안 열리던 것 | ①OTA ②웹 배포 | 🟢 머지 |
| `fix/ops-native-deeplink-20260807` | Android App Links 를 전 경로(`pathPrefix: '/'`)에서 iOS AASA 와 같은 7경로로 축소. **version 1.0.5 → 1.0.6** | 🔴 **새 스토어 빌드**(OTA 불가) | ⏸ **대기 — 머지 = 새 빌드 시점** |

🔴 **`fix/ops-native-deeplink-20260807` 을 그냥 머지하지 마라.** 머지하는 순간 OTA 채널이
1.0.6 으로 옮겨가 1.0.5 기기가 모든 업데이트에서 끊긴다. 새 스토어 빌드를 낼 준비가 됐을 때
함께 처리한다.

⚠️ **배포 순서**: 웹 배포(AASA 갱신분) → OTA → (새 빌드 시점에) 네이티브 브랜치 머지 → 새 빌드
→ 그 다음에야 `ops_hub_enabled` ON. 역순이면 현장 QR 이 죽은 주소로 나가거나 공개 링크가 앱에 갇힌다.

⚠️ **분리의 부작용 하나**: 네이티브 브랜치가 머지되기 전까지 Android 는 여전히 `uniqn.app` 전
경로를 가로챈다(현행 유지). iOS 만 AASA 7경로로 좁혀져 있어 **플랫폼 비대칭이 그동안 지속**된다.
새로 생긴 문제가 아니라 원래 상태이며, 네이티브 브랜치 머지로 닫힌다.
