...
## 7. 🔴 사람 게이트 — prod 마이그레이션 6종 (지금 남은 유일한 코드측 잔여)

> ⚠️ **2026-08-15 재리뷰에서 6번이 추가됐다.** 이 표를 "5종"으로 기억하고 움직이면
> 6번이 빠지고 **공유 계측이 다시 무음으로 죽는다**(§8 참조). 숫자를 다시 세지 말고 표를 볼 것.

이 브랜치는 마이그레이션 6개를 만들었고 **전부 prod 미적용**이다. 로컬에서는
...
- **rls-security · sql-correctness 2축은 끝내 실행되지 못했다.** fable 크레딧 소진 → opus 재시도는
  세션 한도(KST 9시 리셋). 위 9건은 검증됐지만 **그 2축에 남은 결함은 아무도 안 봤다.**
  prod 적용 전에 이 2축을 돌릴 것.
