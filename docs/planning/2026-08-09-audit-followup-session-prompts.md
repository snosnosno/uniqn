...
### 📤 1.0.7 스토어 제출 (2026-08-12) — 사용자 지시로 **QA 전에** 실행

| 플랫폼 | 제출 ID | 대상 | 상태 |
|---|---|---|---|
| iOS | `d67c170a` | ASC **업로드**(build#45 명시 지정) · API Key `MJ98CGZ827`(EAS 서버) | ✅ **finished** (16:18:44 시작) |
| Android | `5a3f97c5` | track **`uniqn 1` = 프로덕션**(사용자 확정) · `releaseStatus: completed` · versionCode 43 | ✅ **finished** (16:19:01→16:21:37, **2분 36초**) |

🔑 **빌드 ID 를 명시해 제출했다**(`--id`) — "최신 빌드 자동선택" 에 맡기지 않았으므로 오늘 만든 그 바이너리가 정확히 올라갔다.
ℹ️ Android 업로드가 이번엔 **2분 36초**였다(1.0.6 때는 23분). 큐 사정이지 설정 차이가 아니다.

🔑 **`eas submit --platform ios` 는 심사 제출이 아니다 — ASC 업로드까지다.** 그래서 1.0.6 심사 중에도 실행할 수 있었다.
실제 심사 제출은 ASC 웹에서 버전에 빌드를 첨부하고 눌러야 하며, **거기서 동시 심사 1개 제한에 걸린다**.
→ 되돌릴 수 있는 상태이고, **TestFlight 로 `auth-F3` 를 실기기 검증할 수 있다**(같은 `com.uniqn.mobile` 을 덮어쓰므로 §1 시나리오가 그대로 재현된다).

🚨 **Android 는 되돌리기 어렵다.** 프로덕션 트랙 + `completed` + `Rollout: undefined`(단계적 출시 미설정) →
Google 검토 통과 즉시 **100% 롤아웃**된다. 실기기 QA 1순위(`auth-F3` 세션 저장소 교체)가 **미검증 상태로 나간다** —
실패하면 1.0.7 로 올라온 사용자가 한 번 로그아웃된다. 검토가 수 시간~수 일 걸리므로 **그 사이 Play Console 에서 halt 하거나 단계적 출시로 낮출 여지는 남아 있다.**

💡 **다음 릴리스 개선 후보**: `eas.json` 의 `submit.production.android` 에 `rollout`(예: `0.1`)을 넣으면
기본이 단계적 출시가 된다. 지금은 없어서 매번 100% 즉시다.
...
6. ✅ **`eas submit` 완료 (08-12)** — iOS `d67c170a` / Android `5a3f97c5`, 둘 다 `finished`. 위 §스토어 제출 참조
     🔑 **정정**: "1.0.6 심사 중이면 1.0.7 을 제출할 수 없다" 는 **`eas submit` 에는 해당하지 않는다**.
        `eas submit --platform ios` 는 **ASC 업로드**까지이고 심사 제출이 아니다 — 동시 심사 1개 제한은
