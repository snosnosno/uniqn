...
| **A3 알림 타입 정합** | ✅ **구현 완료 — 브랜치 `fix/notification-type-parity` `59af9bf0e`** (아래 "A3 교정" 참조). 🔴**prod 마이그 미적용 · PR 미생성** |
...
**잔여**: 🔴prod 마이그 적용 · 🔴PR · 🔴웹배포/OTA · 🔴실기기 QA ·
...
| ~~S+1~~ | ~~**A3 알림 타입 정합**~~ | ✅ **구현 완료 — `fix/notification-type-parity` `59af9bf0e`.** 실제 해법은 "클라 흡수"가 아니라 **DB 트리거 복원**이었다(위 교정 ② 참조). 잔여=prod 적용·PR·배포 |
