...
### 설계 개정 (2026-08-07 — 실사용 확인 후 사용자 결정)

리뷰 결과와 실제 화면을 밟아 본 뒤 내린 결정이다. 위 표(이탈)와 달리 **설계 자체를 바꾼 것**이라 여기 따로 적는다.

| # | 개정 | 근거 |
|---|---|---|
| F11″ | 예외 시트 "**0개 선택 = 확인 잠금**" → "**0개 선택 = 카드 전체 적용**" | 0개가 가장 흔한 조작(전 날짜 같은 조건)인데 그걸 잠그면 날짜 확정 직후 연쇄가 막다른 길이 된다. **0개 선택으로 연다**는 계약은 유지 — 기본 선택을 깔면 훑어보다 그대로 확인해 의도치 않은 분리가 생긴다 |
| F7①③″ | 예외 진입로 2종(카드 버튼 · 시트 하단 링크) → **조건 시트 하나로 통합** | 시트가 "적용할 날짜"를 항상 보여주면 일부만 고르는 것이 곧 예외 추출이다. 진입로가 가리킬 다른 목적지가 없어져 `mode` 전환·리마운트·편집값 승계(구 `seedSlots`) 문제가 **구조적으로 소멸**한다. 순수 연산도 `extractException` → `applyConditionToDates` 하나로 합쳐진다 |
| F11‴ | "적용할 날짜" 후보 = 카드 날짜 → **날짜 없는 카드는 공고 전체 날짜** | 리뷰 HIGH 1 해소. 템플릿 프리셋이 만드는 "조건만 있고 날짜 없는 카드"는 후보가 비어 영영 못 채웠고, 제출 유도 CTA 가 같은 자리를 무한히 가리켰다. 그 카드만 후보를 넓혀 여기서 배정받는다(`requiresDatePick` — 이때는 최소 1개 필수) |
| §3.8″ | 카드 조건 요약 = 한 줄 합산 → **시간대(슬롯)마다 한 줄** | 구 표기 `미정 · 19:00 · 딜러 110` 은 역할을 전 슬롯 합산해 "어느 시간대에 몇 명"을 지웠다. 시간대별로 인원을 나눠 뽑는 것이 대회사의 실제 용도다 → `미정 딜러 50` / `19:00 딜러 60 플로어 10` |
| §4.2″ | 역할 인원 상한 **99 → 999** | 대회사 대규모 편성(딜러 100명+)이 타깃. 스키마 `count.max`·`RoleCountEditor` clamp·`TextInput maxLength` **세 곳이 함께** 움직여야 한다(하나만 남으면 반쪽 상한). 레거시 `jobPosting.schema` 의 100 도 정합 |
