...
| **B2** | 주소 2단계 | `feat/posting-geocoding` | 🔨 **PR 생성** | **#411** | 지오코딩 EF + `geo_lat`/`geo_lng` 컬럼 + 길찾기 좌표 핀. 🔴**마이그 prod 적용 완료 — 재적용 금지**(기록명 `20260803015905`, 레포 파일명 `20260803160000`과 다름). 🔴**EF `geocode-address` prod 선배포(v3)** · 🔴`config.toml` 변경 = 머지 시 **EF 16개 전체 재배포**. 최신 master(#408·#409·#410) 재통합 후 재검증: quality **0 errors** · B2 테스트 **50/50** · 파리티 `PARITY_EXPECT_FUNCS=193` 일치(B2 함수 기여 0). 리뷰 security/code 양쪽 **APPROVE** |
| **S6** | 3-C 설계 | ~~`docs/3c-time-change-design`~~ | ✅ **설계 완료** | | **코드 변경 0건**(설계 전용). 사용자 결정 3건 확정 — ①동의 불필요(알림·동기화만) ②거절=**기존 취소요청 경로** ③**전체가 개인 조정을 덮어쓴다**. 산출물=`docs/planning/2026-08-03-3c-posting-time-change-design.md`. 🔑 **핵심 발견: 알림(Case 2-B #382)·취소 힌트·단건 동기화(#407)가 이미 전부 존재** — 3-C 는 신규 기능이 아니라 묶음 적용이다. 🔴 **§7 미결 1건**(목적지 슬롯 정원 충돌)은 S7 착수 전 사용자 확인 필요 |
| **S7** | 3-C 구현 | `feat/posting-time-change` | ⬜ | | S6 설계 완료 — 프롬프트는 §3 S7 블록 |
