...
### B2 (주소 검색 2단계 — 좌표) — 2026-08-03 · 상태: 구현 완료 (**PR 미생성 — 사용자 결정 대기**)

- 워크트리/브랜치: `C:/Users/user/Desktop/T-HOLDEM-address` / `feat/posting-geocoding` · 베이스 `96dce693a` · 4커밋
...
9. ⚠️ **이 EF 는 인증만 보고 역할은 안 본다** — 구직자 계정도 호출할 수 있다(분당 20회 제한). 카카오 일 10만 쿼터 대비 위험은 낮다고 판단했으나, 역할 게이트를 걸려면 **JWT `app_metadata.role` 이 갓 승격된 employer 에게 stale** 이라는 부작용을 같이 설계해야 한다(그래서 안 걸었다).
