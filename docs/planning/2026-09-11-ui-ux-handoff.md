...
브랜치 docs/rn-list-performance-rules (master 대비 7커밋, HEAD 8cc0a2e83).
docs/planning/2026-09-11-ui-ux-handoff.md 를 먼저 읽어라.

지금 상태: 밀도 축은 소진됐고, 시각 언어(카드 elevation·아바타) 두 건을 넣었는데
둘 다 화면을 못 본 채 넣었다. 실기기 확인이 다음 게이트다.
...
**시각 변경 두 건을 화면을 못 본 채로 넣었다.** 이 환경에 RN 시뮬레이터가 없다.
...
### radius — "51곳 위반"은 오해였다

`rounded-full` 51곳 중 진짜 위반은 **아바타 2곳뿐**이었고 이미 고쳤다. 나머지 49곳은
필터·선택 칩(pill), 카운트 배지, 안읽음 dot(`w-1.5 h-1.5`), 라디오·체크 원 — 전부 원형이
자연스러운 자리다. `Avatar.tsx` 는 이미 `rounded-sm` 으로 규칙을 지키고 있었다.

- `DESIGN.md` 의 "rounded-full 금지, 아바타 포함" 문구를 현실에 맞게 좁힐지는 **미결 판단**
...
2. **PR 생성** — 7커밋을 묶어 올린다 (아직 push 안 함)
...
4. **`DESIGN.md` rounded-full 문구 정정** — 규칙 51 : 코드 1 로 어긋나 있어 규칙이 죽어 있다
...
- **`MobileHeader.tsx` 가 죽은 코드** — 사용처 0. knip 대상 여부 확인 필요.
  그 안에 `useAnimatedStyle` 로 `height` 를 애니메이트하는 룰 위반이 2건 있으나 실효 없음
