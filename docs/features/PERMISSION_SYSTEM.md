...
> **⚠️ 주의: 이 문서는 레거시 웹앱(app2/) 기준의 권한 체계입니다.**
>
> - 이 문서의 `manager` 역할은 현재 모바일앱에서 `employer`로 통합되었습니다.
> - **모바일앱 개발 시**: [CLAUDE.md](../../CLAUDE.md)의 "역할 체계" 섹션을 참조하세요.
> - **Firestore 스키마**: [DATA_SCHEMA.md](../reference/DATA_SCHEMA.md)에서 정확한 필드명을 확인하세요.
>
> | 이 문서 (레거시) | 현재 코드 (모바일앱) | 비고 |
> |----------------|-------------------|------|
> | `admin` | `admin` | 동일 |
> | `manager` | `employer` | 통합됨 (하위호환 코드 제거됨) |
> | `staff` | `staff` | 동일 |
