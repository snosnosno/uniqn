...
## 🟢 집행 현황 (2026-08-13, `chore/cleanup-batch1-20260813` 커밋 7개)
...
| §E5 문서 아카이브 | 🟡 부분 | 링크 0인 22편만 이동. **`docs/archive/*` gitignore 함정 발견·제거**(아카이브가 조용히 실패하던 원인) |
| §E6 브랜치 정리 | ✅ 완료 | 6 → 2 (머지 확인 후 5건 삭제, PR 없던 2건은 아카이브 태그) |
| §C2 배럴 축소 | ⏸ 미착수 | 미사용 export 2,183 → **2,079**(부수효과). 래칫도 2,189 → 2,079 로 조임 |
| §C4 중복 통합 4쌍 | ⏸ 미착수 | 정산 계산 계열 — 등가성 테스트 선행 필요(MED 위험) |
| §C3 아이콘 별칭 20쌍 | ⏸ 미착수 | codemod 동반 필요 |
| §E5 잔여 24편 | ⏸ 보류 | 인바운드 링크가 있어 참조 정리와 동반해야 함 |

기계 증거(브랜치 tip): `tsc --noEmit` EXIT 0 · `npm run quality` EXIT 0(에러 0)
· `jest` 663 suites / 7,546 tests 전량 통과 · `npm run knip:gate` EXIT 0.
