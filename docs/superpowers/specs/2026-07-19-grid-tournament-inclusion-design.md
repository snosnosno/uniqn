...
이유: 셀에 표식을 그리려면 `get_venue_grid_summary`가 날짜별 "대회 포함 여부"를 반환해야 하고, 이는 §5의 **"RPC 응답 컬럼 변경 0"과 충돌**한다. 날짜를 탭하면 상세 패널의 슬롯 행에 `대회` 배지가 이미 뜨므로 정보는 한 탭 거리에 있다.

재검토 조건: D-7 대회가 몰리는 주에 "평소 운영 vs 대회" 구분이 실사용에서 실제 문제로 확인되면 RPC 컬럼 추가와 함께 별건으로 다룬다.
