...
> ⛔ **폐기 (2026-07-19)** — 아래 두 항목이 전제한 홈 대시보드 자체가 PR #276 으로 전면 삭제되어
> 대상 기능이 소멸했다. 재개하려면 홈 화면 재도입 설계부터 필요하다. 결정 이력 보존을 위해 본문은 남긴다.
...
## 공고 작성 일정 UX (2026-08-06 autoplan — 조건 유도 그룹핑 설계에서 이연)

### 기간 템플릿 프리셋 (기간으로 날짜 선택)

- **What**: 날짜 선택 모달에 "기간으로 선택" 모드 — 시작일~종료일 2탭으로 연속 날짜 일괄 선택.
- **Why**: 타깃(대회사 D-7~D-day)의 최빈 흐름은 "연속 기간 + 매일 동일 조건"인데 현재는 7일을 하나하나 탭. CEO 리뷰가 "이 화면 전체보다 활성화에 더 기여할 수 있다"고 평가한 항목.
- **Pros**: 대회사 온보딩 마찰 직격 감소(7탭→2탭), 조건 유도 그룹핑과 직교라 독립 출하 가능.
- **Cons**: CalendarPicker에 range 모드 추가 필요, 급구 7일 제한과의 상호작용 설계 필요.
- **Context**: `CalendarPicker`(multiSelect)가 이미 있고 range 선택만 없음. 조건 유도 그룹핑 재설계(docs/planning/2026-08-06-condition-derived-grouping-design.md)와 무관하게 착수 가능. 효과 측정은 같은 설계의 관측 이벤트(§8.6) 위에 이벤트 1개 추가로 가능.
- **Effort**: M (human ~2일 / CC ~1세션) · **Priority**: P2 · **Depends on**: 없음

### 공고 복제 버튼

- **What**: 발행/마감된 공고 상세에 "이 공고 복제" — 주문서를 해당 공고 값으로 시드해 재발행.
- **Why**: 업계 표준(당근알바 등)은 "조건이 다르면 공고를 나눈다". 상시 알바를 반복 모집하는 홀덤펍 사장의 재사용 흐름이 현재 없음. CEO 리뷰가 "다중 조건 단일 주문서의 대체재" 관점에서 기각 근거 수립을 요구한 항목 — 복제가 있으면 복잡한 조건 분화 수요 자체가 줄어드는지 관찰 가능.
- **Pros**: 재발행 마찰 제거, 프리셋(마지막 공고 시드)보다 명시적 발견성.
- **Cons**: 마감/삭제 공고의 stale 데이터(장소·급여) 시드 검증 필요, 프리셋 캐러셀과 역할 중복 정리 필요.
- **Context**: `PresetCarousel`이 이미 "마지막 공고" 시드를 제공하나 공고 상세에서의 직접 진입은 없음. draft 시드 경로(`draftToValues`)를 재사용하면 됨.
- **Effort**: M (human ~2일 / CC ~1세션) · **Priority**: P3 · **Depends on**: 없음
