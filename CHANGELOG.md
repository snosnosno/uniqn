...
- **트리거 전용 SECDEF 함수 33개의 PUBLIC EXECUTE 회수** (PR #367, prod 마이그 `revoke_public_execute_trigger_functions` 적용완료): Supabase 어드바이저는 "`anon` 이 `/rest/v1/rpc/<name>` 으로 실행 가능"이라 경고했지만, 실제 ACL 은 `=X/postgres` — **grant 주체가 `anon` 이 아니라 PUBLIC** 이었다. `REVOKE ... FROM anon` 만 했다면 PUBLIC 경유 권한이 그대로 남아 **아무것도 안 바뀐 채 "고쳤다"** 가 됐을 것이다. 안전 근거는 두 가지다. ①트리거 함수는 테이블 소유자 권한으로 실행되므로 role 의 EXECUTE 권한과 무관하다 — 회수해도 트리거는 그대로 돈다. ②같은 성격의 SECDEF 트리거 함수 16개(`fn_ops_*`, `prevent_identity_flag_self_update` 등)는 **이미 PUBLIC/anon 이 회수된 상태**였다. 즉 프로젝트가 이미 확립한 패턴이고 초기 33개만 누락돼 있었다. `service_role`/`postgres` 의 EXECUTE 는 유지. 실측: anon 실행가능 33 → **0**, service_role 48/49 유지, 보안 어드바이저 187 → 119건(anon 노출 43→10). 함께 `_posting_slot_key`/`_posting_role_key` 의 search_path 를 고정했다(둘 다 SECURITY INVOKER + `btrim`/`regexp_split_to_array` 뿐인 순수 문자열 함수라 참조하는 스키마 객체가 없다).
- **레포에 없는 Edge Function `create-qa-users` 삭제** (PR #367 부수, 레포 변경 0): 프로덕션에 ACTIVE 였으나 `supabase/functions/` 에는 없었다(대시보드 임시 배포, 2026-04-12 이후 3개월 방치). `verify_jwt:false` + 하드코딩 헤더 `x-qa-secret` 만으로 service_role 을 써 **admin 계정을 고정 비밀번호로 발급**하는 공개 엔드포인트였다. **git 에 없으니 grep·코드리뷰·보안스캔 어디에도 걸리지 않았다** — `list_edge_functions` 와 레포 디렉터리를 주기적으로 대조해야 한다. 레포 참조 0건(Bash grep + Grep 도구 교차검증), `auth.users` 에 해당 계정 0건을 확인하고 삭제했다(EF 16→15, 남은 15개는 레포와 일치). 유령 시크릿 `REVENUECAT_WEBHOOK_SECRET` 도 함께 제거했다(지갑/IAP 는 #196~206 으로 전면 제거됨, 살아있는 참조 0건, 시크릿 10→9).

### Added

- **핵심 퍼널 이벤트 서버 영속화 + 관리자 DAU 실집계** (2026-09-23, 마이그 `20260923100000`, ⚠️ prod 미적용 — 머지 후 적용): `trackEvent` 가 Sentry 브레드크럼만 남겨 가입·지원·공고 등록이 서버에 한 건도 쌓이지 않았고, 관리자 통계의 DAU 는 날짜만 채운 채 0 으로 박혀 있었다(`adminService.ts` "DAU 미구현"). `analytics_events` 화이트리스트에 핵심 퍼널 8종(signup·login·job_view·job_apply·job_create·check_in·check_out·settlement_complete)을 열었다 — **RLS 무변경**, 로그인 사용자 전용이고 anon 정책은 넓히지 않았다. 서버에는 id·method·role·count 화이트리스트 속성만 싣는다(제목·검색어·금액은 브레드크럼 전용 — 보존 기한 없는 계측 테이블이라). DAU 는 신설 RPC `get_admin_daily_active_users(p_days, p_tz)`(SECDEF + admin 게이트, anon EXECUTE 회수, 빈 날 0 채운 연속 날짜)가 집계하고, **조회 실패는 0 이 아니라 null(측정 불가)** 로 화면에 따로 표시한다. 부수 결함 2건: `trackJobCreate` 는 정의만 있고 **호출부가 0** 이었다 / `job_view` 가 재조회마다 재발화해 사용자당 시간당 240건 공용 상한을 소모했다 → 공고당 1회. 파리티 함수 225 → **226**. 검증: 로컬 PG16 스텁에 원본 테이블부터 5개 마이그 체인 적용 + pgTAP 신규(`analytics_core_funnel_dau.test.sql`)가 게이트 제거·이벤트 누락 뮤턴트에서 red · `npm run quality` EXIT 0 · jest 693 스위트 7772 테스트 통과. 한계: DAU 는 백그라운드 복귀만 한 날을 세지 않는다(과소 추정).
- **prod 런타임 헬스 일간 감시** (PR #368, `.github/workflows/prod-health.yml`): 아래 Fixed 의 결함이 두 달간 발각되지 않은 이유는 감시 부재였다. Edge Function 에 Slack 알림 코드는 있었지만 `SLACK_OUTBOX_ALERT_WEBHOOK` 이 **한 번도 설정된 적이 없어**(시크릿 실측) 작성된 순간부터 dead 였고, 크론은 매분 실행돼 `cron.job_run_details` 에 전부 succeeded 로 기록됐다. **실행 성공은 업무 성공이 아니다.** 감시를 앱 밖에 뒀다 — `parity-smoke.yml` 이 이미 `PROD_DB_URL` 시크릿으로 prod 에 붙어 실패 시 GitHub 이 알리는 경로를 만들어 뒀으므로 그 패턴을 재사용해 **새 자격증명 0개**로 끝냈다(새 알림 타입은 클라 착지 처리가 필요해 채택하지 않았다 — PR#365 교훈). 분업은 `parity-smoke`=스키마 드리프트(주간, 변화가 느림) / `prod-health`=런타임 상태(일간, 하루 방치도 길다). 검사 4종(전부 정상값 0): ①`failed_retry_limit` ②`processing` 10분 초과(워커 사망) ③`pending` 1시간 초과(배수 미달) ④크론 실패 24h. ④는 `cron` 스키마 권한이 없으면 `ON_ERROR_STOP=1` 탓에 쿼리 전체가 죽어 **건강 문제가 아닌데 red** 가 되므로 `has_schema_privilege` 가드로 -1(skip)을 반환한다 — 오경보는 감시를 무시하게 만들어 없느니만 못하다. 게이트 red-green 검증: `"35 0 0 0"`(사건 당시 값)→exit 1 · `"0 0 0 -1"`→exit 0 · `"4 0 0 -1"`→exit 1 · `"0 0 0 0"`→exit 0. 첫 실행 실환경 실측 `outbox: failed=0 stuck=0 backlog=0 | cron_failed_24h=0`.
...
- **DB 431MB → 27MB — 로그 테이블 보존 정책 신설** (PR #367, prod 마이그 `cron_run_details_retention` 적용완료): 프로덕션 DB 431MB 중 **410MB(95%)가 로그**였다 — `cron.job_run_details` 218MB(158,630행)와 `net._http_response` 192MB(**362행**, 전부 블로트). public 스키마 실데이터는 4.1MB 다. 원인은 `sync-schedule-board-outbox` 크론이 매분 실행(주 10,080회)되는데 pg_cron 이 실행 이력을 자동으로 지우지 않는 것이고, 결정적으로 두 테이블 모두 `pg_stat_all_tables.last_autovacuum IS NULL` — **autovacuum 이 한 번도 돈 적이 없었다.** 7일 보존 크론 `purge-cron-run-details` 를 신설하고 일회성 `VACUUM (FULL, ANALYZE)` 를 수행했다(트랜잭션 안에서 실행할 수 없어 마이그레이션이 아닌 운영 명령으로 분리). 결과: `cron.job_run_details` 218MB→5.9MB, `net._http_response` 192MB→528kB(362행 유지). 파리티 불변(funcs 183 / policies 111 / pg_temp_missing 0). **안전하지 않다고 판단해 제외한 것**: 미사용 인덱스 45개(`idx_scan=0` 이 트래픽 부재 탓일 수 있다 — users 25명 규모) · 빈 버킷 8개/빈 테이블 16개(미출시 기능일 가능성) · `job_postings` SELECT 정책 3중 중복(접근 범위가 실제로 바뀐다) · `ops_public_reports.opr_insert` WITH CHECK always true.
...
- **주문서 미설정 항목 연쇄 입력**: 공고작성에서 미설정 항목의 `확인`을 누르면 목록 복귀 없이 다음 미설정 항목 시트로 이어진다. 순회는 `nextUnsetRowAfter`(current 다음부터 순환, 한 바퀴 돌면 종료 — 확인 후에도 unset 인 행의 무한 재오픈 차단), 무장 판정은 "미설정 행으로 열었을 때만"이라 이미 채워진 행 수정은 단발 편집으로 남는다. 전환 연출은 `SheetChainContext`로 `SheetModal`에만 전달(시트 12개 무수정) — 180ms 전환 창 동안 같은 농도 딤을 유지해 밝은 목록 번쩍임을 제거하고, 다음 시트는 슬라이드 대신 제자리 fade 로 진입한다. 딤 해제는 `onEntered`(네이티브 `onShow`/웹 rAF) 통지 + 날짜 시트(`DatePickerModal` 래핑, Context 미소비) 경로·예약 취소 경로 안전망 2중.
  - 후속 결함 2건 수정 (PR #307): ①역할 0개/시간 미선택 슬롯 확정을 시트 확인 `disabled`로 근원 차단(급여 시트 데드엔드 해소) ②확정 지원자 잠금 시 잠긴 행을 연쇄 순회에서 그룹 불문 스킵(`skipKeys` — 조기 종료 해소).
  - 폴리시·a11y (PR #308): 연쇄 전환 딤을 호스트로 위임해 상단 헤더까지 커버(`OrderSheetChainScrim` + `onChainSwappingChange`, 헤더 번쩍임 제거) · 전환 시 스크린리더 "다음 항목" 안내 · 웹 연쇄 진입 시 새 시트 제목으로 접근성 포커스 이동 · 연쇄 완료 시 성공 햅틱 · 스왑 대기 플랫폼 분기(웹 0ms) · 슬롯 완성 판정 단일 소스화(빈 배열 진공참 차단). CTA 라벨 예고("다음: 장소")와 Reduce Motion 분기는 검토 후 의도적 무변경(기존 전환이 이미 reduce-motion 친화).
