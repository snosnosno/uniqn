...
금융위원회 유권해석:
  - "특정 사업자의 재화/용역에만 사용 가능한 포인트"
  - "환급 불가, 양도 불가" 조건 시 규제 대상 아님
  - 단, 규모/영향력 확대 시 재검토 필요
```

### 권장 구현 전략

#### Option A: IAP 없이 진행 (권장)

```yaml
전략:
  - 칩 구매 기능 없이 출시
  - 무료 칩만 제공 (가입 보너스, 이벤트 등)
  - 마켓플레이스 모델로 Apple/Google에 설명
...
```yaml
전략:
  - Apple/Google IAP로 칩 구매 기능 구현
  - 수수료 포함 가격 책정

구현 시 고려사항:
  칩 가격표 (예시):
    - 100칩: $0.99 (Apple 몫 $0.30)
    - 500칩: $3.99 (Apple 몫 $1.20)
    - 1000칩: $6.99 (Apple 몫 $2.10)
...
Q1: "칩은 무엇인가요?"
A1: |
  칩은 구인공고 등록 시 플랫폼 이용료로 사용되는 포인트입니다.
  - 현금 환급 불가
  - 타 사용자 양도 불가
  - 가입 시 무료 제공
